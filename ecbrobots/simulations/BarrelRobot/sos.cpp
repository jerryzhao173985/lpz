...
Sos::Sos(double init_feedback_strength) :
  ...
  addParameterDef("dampA", &dampA, 0.003, 0, 0.01, "damping of model");
...
void Sos::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void Sos::stepNoLearning(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void Sos::learn() {
  ...
  b += (xsi * epsA + (b * (-dampA/3.0)) * (epsA > 0 ? 1 : 0)).mapP(0.1, clip);
  ...
  C_update = ((mue * (v ^ T)) + ((mue & y & zeta) * (-2) * (x ^ T))) * (EE * epsC);
