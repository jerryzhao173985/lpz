...
void SRMotorValueWidget::updateViewableChannels()
{
...
//       std::cout << "SRMotorValueWidget: add motorSpeed(" << (*i)->getChannelName() << ") = " << convertToByte((*i)->getValue()) << std::endl;
      ...
      processMotorArrowParams( motorCspeedY, motorCspeedX );
      
    }
    ///Motor-Current-Channel
    if ( dynamic_cast<MotorCurrentPlotChannel*> ( *i ) ) {
//       std::cout << "SRMotorValueWidget: add motorCurrent(" << (*i)->getChannelName() << ") = " << convertToByte((*i)->getValue()) << std::endl;
//       motorCurrent_sensorList.push_back( (*i)->getValue() );
      double motorcurrentX, motorcurrentY;
      if ((*i)->getChannelName() == "motorcurrentX") motorcurrentX = (*i)->getValue();
      if ((*i)->getChannelName() == "motorcurrentY") motorcurrentY = (*i)->getValue();
    }
    ///Tilt-Sensor-Channel
    if ( dynamic_cast<TiltPlotChannel*> ( *i ) ) {
//       std::cout << "SRMotorValueWidget: add tilt(" << (*i)->getChannelName() << ") = " << convertToByte((*i)->getValue()) << std::endl;
//       tilt_sensorList.push_back( (*i)->getValue() );
      double tiltX, tiltY;
      if ((*i)->getChannelName() == "tiltX") tiltX = (*i)->getValue();
      if ((*i)->getChannelName() == "tiltY") tiltY = (*i)->getValue();
      processTiltArrowParams( tiltX, tiltY );
    }
  }
...
void SRMotorValueWidget::setMotorArrow()
{
...
void SRMotorValueWidget::setTiltArrow()
{ 
...
void SRMotorValueWidget::processMotorArrowParams(double m1, double m2)
{
...
void SRMotorValueWidget::processTiltArrowParams( double t1, double t2 )
{
...
//   }
    ...
    // scale len into viewable size
    tilt_arrow.angle = ( ( atan2( t2,t1)) * 180 / PI ) + 90.;
    setTiltArrow ();
  
}


void SRMotorValueWidget::mouseMoveEvent(QMouseEvent * e)
{
  int newOutputX = ( ((e->x()) * 255) / this->width());
  int newOutputY = ( ((e->y()) * 255) / this->height());
  
  std::cout << "new Values X= " << newOutputX << "Y= " << newOutputY << std::endl;
}
