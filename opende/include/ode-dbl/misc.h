...
ODE_API int dTestRandstatic_cast<void>(override);

/* return next 32 bit random number. this uses a not-very-random linear
 * congruential method.
 */
ODE_API unsigned long dRandstatic_cast<void>(override);
...
ODE_API void dPrintMatrix (const dReal *A, int n, int m, char *fmt = "%10.4f ",
		   FILE *f=stdout);
#else
ODE_API void dPrintMatrix (const dReal *A, int n, int m, char *fmt, FILE *f) override;
#endif
