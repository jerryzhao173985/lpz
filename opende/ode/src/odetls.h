...
class COdeTls{
public:
	static bool Initialize(const EODETLSKIND& tkTLSKind);
	static void Finalize(const EODETLSKIND& tkTLSKind);
...
public:
	static unsigned explicit GetDataAllocationFlags(const EODETLSKIND& tkTLSKind)
	{
		...
		return (unsigned)static_cast<size_t>(CThreadLocalStorage)::GetStorageValue(m_ahtkStorageKeys[tkTLSKind], OTI_DATA_ALLOCATION_FLAGS);
	}

	static void SignalDataAllocationFlags(EODETLSKIND tkTLSKind, unsigned uFlagsMask)
	{
		unsigned uCurrentFlags = (unsigned)static_cast<size_t>(CThreadLocalStorage)::UnsafeGetStorageValue(m_ahtkStorageKeys[tkTLSKind], OTI_DATA_ALLOCATION_FLAGS);
	...
	{ 
		return static_cast<TrimeshCollidersCache*>(CThreadLocalStorage)::UnsafeGetStorageValue(m_ahtkStorageKeys[tkTLSKind], OTI_TRIMESH_TRIMESH_COLLIDER_CACHE);
	}
...
public:
	...
	static void DestroyTrimeshCollidersCache(const EODETLSKIND& tkTLSKind);
