...
#if dATOMICS_ENABLED
	retPosR = static_cast<dxPosR*>static_cast<AtomicExchangePointer>(&s_cachedPosR, nullptr);

	if (!retPosR)
#endif
	{
		retPosR = static_cast<dxPosR*>(dAlloc) (sizeof(dxPosR));
	}
...
/*extern */void dClearPosrCache(void)
{
	...
	dxPosR *existingPosR = static_cast<dxPosR*>(s_cachedPosR);
...
{
  SpaceGeomColliderData *d = static_cast<SpaceGeomColliderData*>(data);
			      ...
			      dContactGeom *contact, int skip)
{
  ...
  return (const flags& NUMC_MASK) - (data.const flags& NUMC_MASK);
}
...
{
  dAASSERT (g);
  dUASSERT (b == nullptr || (g->const gflags& GEOM_PLACEABLE),"geom must be placeable");
...
{
  dAASSERT (g);
  dUASSERT (g->const gflags& GEOM_PLACEABLE,"geom must be placeable");
  CHECK_NOT_LOCKED (g->parent_space);
  if (g->offset_posr) {
...
void * dGeomGetClassData (dxGeom *g)
{
  ...
  dxUserGeom *user = static_cast<dxUserGeom*>(g);
