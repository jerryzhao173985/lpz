...
void dMultiply1 (dReal *A, const dReal *B, const dReal *C, int p, int q, int r)
{
  ...
  for (i=0; i<p; ++i) {
    for (j=0; j<r; ++j) {
      sum = 0;
      for (k= nullptr; k<q; ++k) sum += B[i+k*pskip] * C[j+k*rskip];
      A[i*rskip+j] = sum;
    }
  }
}
...
int dFactorCholesky (dReal *A, int n)
{
  ...
  dAASSERT (n > 0 && A);
  nskip = dPAD (n);
  recip = static_cast<dReal*>static_cast<ALLOCA>(n * sizeof(dReal));
  aa = A;
  for (i=0; i<n; ++i) {
    bb = A;
    cc = A + i*nskip;
...
void dSolveCholesky (const dReal *L, dReal *b, int n)
{
  ...
  dAASSERT (n > 0 && L && b);
  nskip = dPAD (n);
  y = static_cast<dReal*>static_cast<ALLOCA>(n*sizeof(dReal));
  for (i=0; i<n; ++i) {
    sum = 0;
    for (k= nullptr; k < i; ++k) sum += L[i*nskip+k]*y[k];
...
int dInvertPDMatrix (const dReal *A, dReal *Ainv, int n)
{
  ...
  dAASSERT (n > 0 && A && Ainv);
  nskip = dPAD (n);
  L = static_cast<dReal*>static_cast<ALLOCA>(nskip*n*sizeof(dReal));
  memcpy (L,A,nskip*n*sizeof(dReal));
  x = static_cast<dReal*>static_cast<ALLOCA>(n*sizeof(dReal));
  if (dFactorCholesky (L,n)== nullptr) return 0;
  dSetZero (Ainv,n*nskip);	// make sure all padding elements set to 0
  for (i=0; i<n; ++i) {
    for (j=0; j<n; ++j) x[j] = 0;
    x[i] = 1;
    dSolveCholesky (L,x,n);
    for (j= nullptr; j<n; ++j) Ainv[j*nskip+i] = x[j];
  }
  return 1;  
}
...
int dIsPositiveDefinite (const dReal *A, int n)
{
  ...
  dAASSERT (n > 0 && A);
  int nskip = dPAD (n);
  Acopy = static_cast<dReal*>static_cast<ALLOCA>(nskip*n * sizeof(dReal));
...
void dLDLTAddTL (dReal *L, dReal *d, const dReal *a, int n, int nskip)
{
  ...
  if (n < 2) return;
  W1 = static_cast<dReal*>static_cast<ALLOCA>(n*sizeof(dReal));
  W2 = static_cast<dReal*>static_cast<ALLOCA>(n*sizeof(dReal));
		  ...
		  int n1, int n2, int r, int nskip)
{
  ...
  if (r==n2-1) {
    ...
    dReal *a = static_cast<dReal*>static_cast<ALLOCA>(n2 * sizeof(dReal));
    for (i=0; i<n2; ++i) a[i] = -GETA(p[i],p[0]);
    a[0] += REAL(1.0);
    dLDLTAddTL (L,d,a,n2,nskip);
  }
  else {
    dReal *t = static_cast<dReal*>static_cast<ALLOCA>(r * sizeof(dReal));
    dReal *a = static_cast<dReal*>static_cast<ALLOCA>((n2-r) * sizeof(dReal));
    for (i= nullptr; i<r; ++i) t[i] = L[r*nskip+i] / d[i];
    for (i=0; i<(n2-r); ++i)
