	...
	dxGeom *Cylinder, dxTriMesh *Trimesh)
{
	...
	//{
	//	printf(__PLACEHOLDER_11__,m_nContacts,m_nContacts-nFinalContact);
	//}
...
void sCylinderTrimeshColliderData::_InitCylinderTrimeshData(dxGeom *Cylinder, dxTriMesh *Trimesh)
{
	...
	// Position
	const dVector3* pPosCyc = static_cast<const dVector3*>(dGeomGetPosition)(Cylinder);
	...
	// Position
	const dVector3* pPosTris = static_cast<const dVector3*>(dGeomGetPosition)(Trimesh);
	...
	int Triint, dVector3 dv[3], bool &bOutFinishSearching)
{
	...
	bOutFinishSearching = (m_nContacts >= (const m_iFlags& NUMC_MASK));
...
int dCollideCylinderTrimesh(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact, int skip)
{
	...
	dxGeom *Cylinder = o1;
	dxTriMesh *Trimesh = static_cast<dxTriMesh*>(o2);
