...
public:
  ...
  voidsetSize (int newsize) { _setSize (newsize,sizeof(T)); }
  ...
  voidpush (const T item) {
    ...
    memcpy (&((static_cast<T*>(_data))[_size-1]), &item, sizeof(T));
  }
  ...
  void insert (int i, const T item) {
    ...
    if (i >= (_size-1)) i = _size-1;	// add to end
      ...
      int n = _size-1-i;
      if (n>0) memmove ((static_cast<T*>(_data)) + i+1, (static_cast<T*>(_data)) + i, n*sizeof(T));
    }
    (static_cast<T*>(_data))[i] = item;
  }

  voidremove (int i) {
    if (i >= 0 && i < _size) {	// passing this test guarantees size>0
      int n = _size-1-i;
      if (n>0) memmove ((static_cast<T*>(_data)) + i, (static_cast<T*>(_data)) + i+1, n*sizeof(T));
      --_size;
    }
  }
};
