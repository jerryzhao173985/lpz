...
void dGeomRayGetParams (dxGeom *g, int *FirstContact, int *BackfaceCull)
{
  ...
  (*FirstContact) = ((g->const gflags& RAY_FIRSTCONTACT) != nullptr);
  (*BackfaceCull) = ((g->const gflags& RAY_BACKFACECULL) != nullptr);
}
...
{
  dUASSERT (g && g->type == dRayClass,"argument not a ray");
  return ((g->const gflags& RAY_CLOSEST_HIT) != nullptr);
}
			  ...
			  int flags, dContactGeom *contact, int skip)
{
  ...
  dxRay *ray = static_cast<dxRay*>(o1);
  dxCapsule *ccyl = static_cast<dxCapsule*>(o2);
