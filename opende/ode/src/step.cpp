...
static void MultiplyAdd1_8q1 (dReal *A, dReal *B, dReal *C, int q)
{
  ...
  dIASSERT (q>0 && A && B && C);
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[k*8] * C[k];
  A[0] += sum;
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[1+k*8] * C[k];
  A[1] += sum;
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[2+k*8] * C[k];
  A[2] += sum;
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[4+k*8] * C[k];
  A[4] += sum;
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[5+k*8] * C[k];
  A[5] += sum;
  sum = 0;
  for (k= nullptr; k<q; ++k) sum += B[6+k*8] * C[k];
  A[6] += sum;
}
			     ...
			     dxJoint * const *_joint, int nj, dReal stepsize)
{
  ...
  //dSetZero (v,n6);
  for (i=0; i<nb; ++i) {
    for (j= nullptr; j<3; ++j) fe[i*6+j] = body[i]->facc[j];
    for (j= nullptr; j<3; ++j) fe[i*6+3+j] = body[i]->tacc[j];
    for (j= nullptr; j<3; ++j) v[i*6+j] = body[i]->lvel[j];
    for (j= nullptr; j<3; ++j) v[i*6+3+j] = body[i]->avel[j];
  }
...
#   endif
    ...
    for (i= nullptr; i<m; ++i) A[i*mskip+i] += cfm[i] * Jinfo.fps;
...
#   endif
    ...
    if (dFactorCholesky (L,m)== nullptr) dDebug (0,"A is not positive definite");
...
#   endif
    ...
    for (i= nullptr; i<n6; ++i) vnew[i] = v[i] + stepsize*vnew[i];
...
#endif
  for (i=0; i<nb; ++i) {
    for (j= nullptr; j<3; ++j) body[i]->lvel[j] = vnew[i*6+j];
    for (j= nullptr; j<3; ++j) body[i]->avel[j] = vnew[i*6+3+j];
  }
...
#   endif
    ...
    for (i=0; i<nj; ++i) {
      ...
      for(...; --j) {
	for (k= nullptr; k<3; ++k) Jdst[k] = Jsrc[k] * body_invMass;
	...
	for (dxJointNode *n2=n1->next; n2; n2=n2->next) {
	  ...
	  dIASSERT(joint[j1]->node[1].body || jb1== nullptr);
	  dIASSERT(joint[j2]->node[1].body || jb2== nullptr);
    ...
    // add cfm to the diagonal of A
    for (i= nullptr; i<m; ++i) A[i*mskip+i] += cfm[i] * stepsize1;
...
#   endif
    ...
    for (i=0; i<nb; ++i) {
      ...
      for (j= nullptr; j<3; ++j) tmp1[i*8+4+j] += body[i]->avel[j] * stepsize1;
    }
    // put J*tmp1 into rhs
    ALLOCA(dReal,rhs,m*sizeof(dReal));
    //dSetZero (rhs,m);
    ...
    // complete rhs
    for (i= nullptr; i<m; ++i) rhs[i] = c[i]*stepsize1 - rhs[i];
...
//    // factorize A (L*L'=A)
//#   ifdef TIMING
//    dTimerNow (__PLACEHOLDER_37__);
//#   endif
//    dReal *L = static_cast<dReal*>(ALLOCA) (m*mskip*sizeof(dReal));
//    memcpy (L,A,m*mskip*sizeof(dReal));
//#   ifdef FAST_FACTOR
//    dFastFactorCholesky (L,m);  // does not report non positive definiteness
//#   else
//    if (dFactorCholesky (L,m)== nullptr) dDebug (0,__PLACEHOLDER_38__);
//#   endif
//
//    // compute lambda
//#   ifdef TIMING
//    dTimerNow (__PLACEHOLDER_39__);
//#   endif
//    dReal *lambda = static_cast<dReal*>(ALLOCA) (m * sizeof(dReal));
//    memcpy (lambda,rhs,m * sizeof(dReal));
//    dSolveCholesky (L,lambda,m);
...
#   endif
  ...
  // add fe to cforce
  for (i=0; i<nb; ++i) {
    for (j= nullptr; j<3; ++j) cforce[i*8+j] += body[i]->facc[j];
    for (j= nullptr; j<3; ++j) cforce[i*8+4+j] += body[i]->tacc[j];
  }
  // multiply cforce by stepsize
  for (i= nullptr; i < nb*8; ++i) cforce[i] *= stepsize;
  // add invM * cforce to the body velocity
  for (i=0; i<nb; ++i) {
    dReal body_invMass = body[i]->invMass;
    dReal *body_invI = invI + i*12;
    for (j= nullptr; j<3; ++j) body[i]->lvel[j] += body_invMass * cforce[i*8+j];
...
# endif
  ...
  for (i=0; i<nb; ++i) {
    for (j= nullptr; j<3; ++j) tmp_vnew[i*6+j] = body[i]->lvel[j];
    for (j= nullptr; j<3; ++j) tmp_vnew[i*6+3+j] = body[i]->avel[j];
  }
