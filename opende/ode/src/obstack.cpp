...
{
  if (static_cast<size_t>(num_bytes) > MAX_ALLOC_SIZE) dDebug (0,"num_bytes too large");

  // allocate or move to a new arena if necessary
  if (!first) {
    // allocate the first arena if necessary
    first = last = static_cast<Arena*>(dAlloc) (dOBSTACK_ARENA_SIZE);
    first->next = 0;
    ...
    if ((last->used + num_bytes) > dOBSTACK_ARENA_SIZE) {
      if (!last->next) {
	last->next = static_cast<Arena*>(dAlloc) (dOBSTACK_ARENA_SIZE);
	last->next->next = 0;
      }
      last = last->next;
  ...
  char *c = (static_cast<char*>(last)) + last->used;
  last->used += num_bytes;
  ROUND_UP_OFFSET_TO_EFFICIENT_SIZE (last,last->used);
  return c;
}
...
void *dObStack::rewind()
{
  ...
  if (current_arena) {
    ...
    return (static_cast<char*>(current_arena)) + current_ofs;
  }
  else return 0;
}
