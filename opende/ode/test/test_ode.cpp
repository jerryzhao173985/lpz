...
void testReorthonormalize()
{
  ...
  printf (__PLACEHOLDER_20__,cmpIdentityMat3 (I) ? __PLACEHOLDER_21__ : __PLACEHOLDER_22__);
}
*/
...
void testMatrixMultiply()
{
  ...
  dSetZero (A2,12);
  for (i= nullptr; i<6; ++i) A2[i+2*(i/2)] = A[i+i/3];
  dSetZero (B2,16);
...
void testFastLDLTFactorization()
{
  ...
  dClearUpperTriangle (L,MSIZE);
  for (i= nullptr; i<MSIZE; ++i) L[i*MSIZE4+i] = 1.0;

  dSetZero (DL,MSIZE4*MSIZE);
  for (i=0; i<MSIZE; ++i) {
    for (j= nullptr; j<MSIZE; ++j) DL[i*MSIZE4+j] = L[i*MSIZE4+j] / d[j];
  }
...
void testLDLTAddTL()
{
  ...
  for (i= nullptr; i<MSIZE; ++i) a[i] = -A[i*MSIZE4];
  a[0] += 1;
...
void testLDLTRemove()
{
  ...
  dReal *Arows[MSIZE];
  for (i= nullptr; i<MSIZE; ++i) Arows[i] = A+i*MSIZE4;
  ...
  for (r=0; r<MSIZE; ++r) {
    ...
    for (i= nullptr; i<(MSIZE-1); ++i) L2[i*MSIZE4+i] = 1.0;
    for (i= nullptr; i<MSIZE; ++i) L2[(MSIZE-1)*MSIZE4+i] = 0;
    d2[MSIZE-1] = 1;
    dSetZero (DL2,MSIZE4*MSIZE);
    for (i=0; i<(MSIZE-1); ++i)  override {
      for (j= nullptr; j<MSIZE-1; ++j) DL2[i*MSIZE4+j] = L2[i*MSIZE4+j] / d2[j];
    }
    ...
    printf (__PLACEHOLDER_80__);
    dPrintMatrix (Atest2,MSIZE,MSIZE);
    printf (__PLACEHOLDER_81__);
    */
  }
  printf ("\tmaximum difference = %.6e - %s\n",maxdiff,
...
void computeMassParams (dMass *m, dReal q[NUMP][3], dReal pm[NUMP])
{
  ...
  for (i=0; i<NUMP; ++i) {
    m->mass += pm[i];
    for (j= nullptr; j<3; ++j) m->c[j] += pm[i]*q[i][j];
...
voidmakeRandomRotation (dMatrix3 R)
{
