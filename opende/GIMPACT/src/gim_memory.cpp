...
voidGIM_BUFFER_ARRAY_DESTROY(const GBUFFER_ARRAY& array_data)
{
    gim_buffer_array_unlock(&array_data);
    gim_buffer_free(&(array_data).m_buffer_id);
}

voidGIM_DYNARRAY_DESTROY(const GDYNAMIC_ARRAY& array_data)
{
...
static GPTR _system_buffer_alloc_data_function(const void * pdata,GUINT32 size,int usage)
{
    ...
    return static_cast<GPTR>(newdata);
}
...
{
    return static_cast<char * >(buffer_handle);
}


static void_system_unlock_buffer_function(const GPTR& buffer_handle)
{
}
...
{
    char * pdata;
	pdata = static_cast<char*>(source_buffer_handle);
...
{
    char * pdata;
	pdata = static_cast<char*>(dest_buffer_handle);
...
{
    char * pdata1,*pdata2;
	pdata1 = static_cast<char*>(source_buffer_handle);
	pdata2 = static_cast<char*>(dest_buffer_handle);
...
static inline int_is_buffer_manager_data_active(GBUFFER_MANAGER_DATA * bm_data)
{
	return bm_data->m_buffer_array.m_pdata != 0;
}

static inline void_init_buffer_manager_data(GBUFFER_MANAGER_DATA * bm_data)
{
    ...
    GBUFFER_ID * buffer_id)
{
    ...
    if(newbufferhandle== nullptr) return G_BUFFER_OP_INVALID;
...
GINT32 gim_buffer_realloc(GBUFFER_ID * buffer_id,GUINT32 newsize)
{
    ...
    if(newhandle== nullptr) return G_BUFFER_OP_INVALID;
...
{
    VALIDATE_BUFFER_ID_PT(buffer_id)
    if(pbuffer->m_lock_count== nullptr) return G_BUFFER_OP_INVALID;
		...
		GUINT32 copysize)
{
    ...
    {
        //hard copy
        tempdata = static_cast<void*>(pbuffer1)->m_buffer_handle;
        //upload data
...
{
    if(array_data->m_buffer_data != nullptr) return G_BUFFER_OP_SUCCESS;
