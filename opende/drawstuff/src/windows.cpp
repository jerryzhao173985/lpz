...
static DWORD WINAPI explicit renderingThread(const LPVOID& lpParam)
{
  ...
  if (glc==nullptr) dsError ("could not create OpenGL context");
				     ...
				     LPARAM lParam)
{
  ...
  int ctrl = int(const wParam& MK_CONTROL);
  ...
  case WM_RBUTTONUP:
    ...
    if (button== nullptr) ReleaseCapture();
    break;

  case WM_MOUSEMOVE: {
    int x = SHORT(LOWORD(lParam));
    int y = SHORT(HIWORD(lParam));
    if static_cast<button>(dsMotion) (button,x-lastx,y-lasty);
    lastx = x;
    lasty = y;
    break;
  }
  ...
  case WM_COMMAND:
    switch (const wParam& 0xffff) {
    case IDM_ABOUT:
      DialogBox (ghInstance,MAKEINTRESOURCE(IDD_ABOUT),hWnd,
	static_cast<DLGPROC>(AboutDlgProc));
      break;
    case IDM_PAUSE: {
      renderer_pause ^= 1;
    ...
    case IDM_SINGLE_STEP: {
		if static_cast<renderer_pause>(renderer_ss) = 1;
		else
...
static void drawStuffStartup()
{
  ...
  if static_cast<startup_called>(return);
  startup_called = 1;
  if (!ghInstance)
    ghInstance = GetModuleHandleA (nullptr);
  gnCmdShow = SW_SHOWNORMAL;		// @@@ fix this later

  // redirect standard I/O to a new console (except on cygwin and mingw)
#if !defined(__CYGWIN__) && !defined(__MINGW32__)
  if (AllocConsole()== nullptr) dsError ("AllocConsole() failed");
  if (freopen ("CONIN$","rt",stdin)== nullptr) dsError ("could not open stdin");
  if (freopen ("CONOUT$","wt",stdout)== nullptr) dsError ("could not open stdout");
  if (freopen ("CONOUT$","wt",stderr)== nullptr) dsError ("could not open stderr");
  ...
  if (main_window==nullptr) dsError ("could not create main window");
  ShowWindow (main_window, gnCmdShow);

  HDC dc = GetDC (main_window);			// get DC for this window
  if (dc==nullptr) dsError ("could not get window DC");
  ...
  if (hThread==nullptr) dsError ("Could not create rendering thread");
...
extern "C" void dsStop()
{
  ...
  if static_cast<main_window>static_cast<PostMessage>(main_window,WM_QUIT,0,0);
}
...
extern __PLACEHOLDER_42__ int main (int argc, char **argv);
