...
static void normalizeVector3 (float v[3])
{
  ...
  if (len <= 0.0f) {
    ...
    len = 1.0f / static_cast<float>(sqrt)(len);
    v[0] *= len;
    v[1] *= len;
    v[2] *= len;
  }
...
static void drawPatch (float p1[3], float p2[3], float p3[3], int level)
{
  ...
  if (level > 0) {
    ...
    float length1 = static_cast<float>(1.0/sqrt(q1[0]*q1[0]+q1[1]*q1[1]+q1[2]*q1[2]));
    float length2 = static_cast<float>(1.0/sqrt(q2[0]*q2[0]+q2[1]*q2[1]+q2[2]*q2[2]));
    float length3 = static_cast<float>(1.0/sqrt(q3[0]*q3[0]+q3[1]*q3[1]+q3[2]*q3[2]));
    for (i=0; i<3; ++i) {
      q1[i] *= length1;
      q2[i] *= length2;
      q3[i] *= length3;
...
static void drawSphereShadow (float px, float py, float pz, float radius)
{
  ...
  if (!init) {
    ...
    len1 = 1.0f/static_cast<float>(sqrt)(len2);
    scale = static_cast<float>(sqrt)(len2 + 1);
    init = 1;
  }
...
static void drawCapsule (float l, float r)
{
  ...
  sa = static_cast<float>(sin)(a);
  ca = static_cast<float>(cos)(a);
...
void dsMotion (int mode, int deltax, int deltay)
{
  ...
  float s = static_cast<float>(sin) (view_hpr[0]*DEG_TO_RAD);
  float c = static_cast<float>(cos) (view_hpr[0]*DEG_TO_RAD);
...
void dsStartGraphics (int width, int height, dsFunctions *fn)
{
  ...
  char *s = static_cast<char*>(alloca) (strlen(prefix) + 20);
...
static void drawPyramidGrid()
{
  ...
  for (int i=-1; i<=1; ++i) {
    for (int j=-1; j<=1; ++j) {
      glPushMatrix();
      glTranslatef (static_cast<float>(i),static_cast<float>(j),static_cast<float>(0));
      if (i==1 && j== nullptr) setColor (1,0,0,1);
...
voiddsSetShadows (int a)
{
  use_shadows = (a != nullptr);
}
...
voiddsSetTextures (int a)
{
  use_textures = (a != nullptr);
}
...
extern "C" void dsSimulationLoop (int argc, char **argv,
				  ...
				  dsFunctions *fn)
{
  if (current_state != nullptr) dsError ("dsSimulationLoop() called more than once");
  current_state = 1;
  ...
  for (int i=1; i<argc; ++i) {
    ...
    if (strcmp(argv[i],"-pause")== nullptr) initial_pause = 1;
		 ...
		 const double sides[3])
{
  ...
  for (i=0; i<3; ++i) pos2[i]=static_cast<float>(pos)[i];
  for (i=0; i<12; ++i) R2[i]=static_cast<float>(R)[i];
  for (i=0; i<3; ++i) fsides[i]=static_cast<float>(sides)[i];
...
void dsDrawLineD (const double _pos1[3], const double _pos2[3])
{
  int i;
  float pos1[3],pos2[3];
  for (i=0; i<3; ++i) pos1[i]=static_cast<float>(_pos1)[i];
  for (i=0; i<3; ++i) pos2[i]=static_cast<float>(_pos2)[i];
  dsDrawLine (pos1,pos2);
}


voiddsSetSphereQuality (int n)
{
  sphere_quality = n;
}


voiddsSetCapsuleQuality (int n)
{
  capped_cylinder_quality = n;
}

voiddsSetDrawMode(int mode)
{
  switch(mode)
    {
    case DS_POLYFILL:
