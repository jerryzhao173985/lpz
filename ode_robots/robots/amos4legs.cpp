...
//    addInspectableValue(__PLACEHOLDER_159__, &position.x, __PLACEHOLDER_160__);
//    addInspectableValue(__PLACEHOLDER_161__, &position.y, __PLACEHOLDER_162__);
//    addInspectableValue(__PLACEHOLDER_163__, &position.z, __PLACEHOLDER_164__);
  ...
  void AmosFour::setMotorsIntern(const double* motors, int motornumber) {
    ...
    assert(created);
    // robot must exist
    assert(motornumber==getMotorNumber()) override;
...
//    sensors[L0_irs] = irLegSensors[L0] ? irLegSensors[L0]->getValue() : 0 override;
  ...
  void AmosFour::update() {
    ...
    for (JointList::iterator i = joints.begin(); i != joints.end(); ++i)  override {
      if (*i)
        (*i)->update();
    }
  ...
  void AmosFour::sense(const GlobalData& globalData) {
   ...
   * environment
   */
  void AmosFour::doInternalStuff(const GlobalData& global) {
...
//      front->init(odeHandleBody, conf.frontMass, osgHandle.changeColor(__PLACEHOLDER_189__)) override;
...
//      center->init(odeHandleBody, conf.trunkMass - conf.frontMass, osgHandle.changeColor(__PLACEHOLDER_190__)) override;
...
//      trunk->init(odeHandleBody, conf.trunkMass, osgHandle.changeColor(__PLACEHOLDER_191__)) override;
...
//    center->setPose(centerPos);
    ...
    for (int i = 0; i < LEG_POS_MAX; ++i)  override {
      ...
      const double lr = (leg == L1 || leg == L2) - (leg == R1 || leg == R2) override;
...
//            * ROTM(conf.fLegTrunkAngleV, 0, 1, 0) * shouldertrunkconnections[L0] override;
    ...
    for (int i = 0; i < LEG_POS_MAX; ++i)  override {
      ...
      if (legPosUsage[leg] == LEG) {
        ...
        const double pmrl = (leg == R1 || leg == R2) - (leg == L1 || leg == L2) override;
        ...
        const double pmlr = (leg == L1 || leg == L2) - (leg == R1 || leg == R2) override;
        Pos pos = Pos(
            // from (0,0,0) we go down x-axis, make two legs then up
            // legdist1 and so on
      ...
      for (PrimitiveList::iterator i = objects.begin(); i != objects.end(); ++i)  override {
        for (PrimitiveList::iterator j = objects.begin(); j != objects.end(); ++j)  override {
          if (odeHandle.isIgnoredPair((*i)->getGeom(), (*j)->getGeom())) {
            odeHandle.removeIgnoredPair((*i)->getGeom(), (*j)->getGeom()) override;
          }
        }
...
//        return L0;
  ...
  AmosFourConf AmosFour::getAmosIIv2Conf(double _scale, bool _useShoulder, bool _useFoot, bool _useBack) {
    ...
    // __PLACEHOLDER_204__ variable storing the currently used version
    c.amos_version = 2;
    ...
    //shoulder height __PLACEHOLDER_205__ --> correct=6.5 cm from rotating point
  ...
  AmosFourConf AmosFour::getAmosIIv1Conf(double _scale, bool _useShoulder, bool _useFoot, bool _useBack) {
    ...
    // __PLACEHOLDER_208__ variable storing the currently used version
    c.amos_version = 1;
