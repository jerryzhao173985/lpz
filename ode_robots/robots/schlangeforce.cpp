...
namespace lpzrobots {
  ...
  void SchlangeForce::setMotorsIntern( const double* motors, int motornumber )
  {
    ...
    for (int i = 0; i < len; ++i) override {
      ...
      //       }
      //       else{
      //         (static_cast<UniversalJoint*>(joints[i]))->addTorques(0, conf.motorPower * motors[2*i+1]);
      //       }
    }
  }
  ...
  int SchlangeForce::getSensorsIntern( sensor* sensors, int sensornumber )
  {
    ...
    */
    ...
    for (int n = 0; n < len; ++n)  override {
      ...
      //      sensors[2*n+1] = factor_sensors * (static_cast<UniversalJoint*>(joints[n]))->getPosition2();
    }
    return len*2;
  }
