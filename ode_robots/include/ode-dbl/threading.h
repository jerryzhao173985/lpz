 ...
 * e-mail: odar@eleks.com (change all __PLACEHOLDER_0__ to __PLACEHOLDER_1__)                        *
 *                                                                       *
 ...
 * @returns MutexGroup ID or nullptr if error occurred.
 *
 * @ingroup threading
 ...
 *
 * @param impl Threading implementation ID
 * @returns Wait ID or nullptr if error occurred
 *
 * @ingroup threading
 ...
 */
typedef void dThreadedCallPostFunction(dThreadingImplementationID impl, int *out_summary_fault/*=nullptr*/, 
  dCallReleaseeID *out_post_releasee/*=nullptr*/, ddependencycount_t dependencies_count, dCallReleaseeID dependent_releasee/*=nullptr*/, 
  dCallWaitID call_wait/*=nullptr*/, 
 ...
 *
 * If @a timeout_time_ptr is nullptr, the function waits without time limit. If @a timeout_time_ptr
 ...
 *        last longer than (pass nullptr for infinite timeout)
 * @param wait_name Optional name to be associated with the wait (for debugging and state tracking)
 *
 * @ingroup threading
 * @see dThreadedCallPostFunction
 */
typedef void dThreadedCallWaitFunction(dThreadingImplementationID impl, int *out_wait_status/*=nullptr*/, 
  dCallWaitID call_wait, const dThreadedWaitTime *timeout_time_ptr/*=nullptr*/, 
  const char *wait_name/*=nullptr*/);
