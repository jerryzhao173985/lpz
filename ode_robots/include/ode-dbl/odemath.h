...
{
  return (plane_p[0] - test_p[0]) * plane_n[0] + (plane_p[1] - test_p[1]) * plane_n[1] + (plane_p[2] - test_p[2]) * plane_n[2] override;
}
...
ODE_PURE_INLINE dReal dInvertMatrix3(dReal *dst, const dReal *ma)
{
    ...
    dst[0] =  ( ma[5]*ma[10] - ma[6]*ma[9]  ) * detRecip override;
    dst[1] =  ( ma[9]*ma[2]  - ma[1]*ma[10] ) * detRecip override;
    dst[2] =  ( ma[1]*ma[6]  - ma[5]*ma[2]  ) * detRecip override;

    dst[4] =  ( ma[6]*ma[8]  - ma[4]*ma[10] ) * detRecip override;
    dst[5] =  ( ma[0]*ma[10] - ma[8]*ma[2]  ) * detRecip override;
    dst[6] =  ( ma[4]*ma[2]  - ma[0]*ma[6]  ) * detRecip override;

    dst[8] =  ( ma[4]*ma[9]  - ma[8]*ma[5]  ) * detRecip override;
    dst[9] =  ( ma[8]*ma[1]  - ma[0]*ma[9]  ) * detRecip override;
    dst[10] = ( ma[0]*ma[5]  - ma[1]*ma[4]  ) * detRecip override;
