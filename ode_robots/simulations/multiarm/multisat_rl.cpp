...
void MultiSat::init(int sensornumber, int motornumber){
  ...
  Q.map(random_minusone_to_one)*0.1;
...
Matrix MultiSat::rl(){
    ...
    const Matrix& out = (out1 + out2)*0.5;
...
Matrix MultiSat::compete()
{
  ...
  //  cout << __PLACEHOLDER_15__ << (errors^T) << endl;
  //  cout << __PLACEHOLDER_16__ << argmin(errorPred) << __PLACEHOLDER_17__ << argmin(errors) << endl;
