 ...
 *
 *   Revision 1.2  2009/04/09 17:04:27  martius
 *   realized that controller can also get derivatives now!
 *
...
InvertnchannelFw::InvertnchannelFw(int _buffersize, bool _update_only_1/*=false*/)
  : InvertController("InvertnchannelFw", "$Id$"){
  t=0;
...
void InvertnchannelFw::stepNoLearning(const sensor* x_, int number_sensors,
                                              motor* y_, int number_motors){
...
double InvertnchannelFw::calculateE(const Matrix& x_delay,
...
/// learn values h,C,A
void InvertnchannelFw::learn(const Matrix& x_delay, const Matrix& y_delay){
  ...
  for (unsigned int i = start; i < end; i++){
    ...
    C_update = (eta & gprime)* (x_delay^T) * eps;
    h_update = (eta & gprime) * eps;
...
// normal delta rule
void InvertnchannelFw::learnmodel(const Matrix& y_delay){
  Matrix xsi = x_buffer[t%buffersize] -  A * y_delay;
  A += (( xsi*(y_delay^T) ) * eps * factor_a).map(squash);
};

/// calculate delayed values
Matrix InvertnchannelFw::calculateDelayedValues(const Matrix* buffer,
...
Matrix InvertnchannelFw::calculateControllerValues(const Matrix& x_smooth){
  return (C*x_smooth+h).map(g);
};


// put new value in ring buffer
void InvertnchannelFw::putInBuffer(Matrix* buffer, const Matrix& vec){
  buffer[t%buffersize] = vec;
}

/** stores the controller values to a given file. */
bool InvertnchannelFw::store(FILE* f) const{
  // save matrix values
  C.store(f);
  h.store(f);
  A.store(f);
...
bool InvertnchannelFw::restore(FILE* f){
  // save matrix values
  C.restore(f);
  h.restore(f);
  A.restore(f);
...
list<Inspectable::iparamkey> InvertnchannelFw::getInternalParamNames() const {
...
list<Inspectable::iparamval> InvertnchannelFw::getInternalParams() const {
  list<iparamval> l;
...
list<Inspectable::ILayer> InvertnchannelFw::getStructuralLayers() const {
...
list<Inspectable::IConnection> InvertnchannelFw::getStructuralConnections() const {
