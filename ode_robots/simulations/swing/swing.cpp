...
namespace lpzrobots {
    ...
    : OdeRobot(odeHandle, osgHandle, name, "$Id$"), conf(c)
  {
    ...
    explicit if(conf.onlyPrimaryFunctions){
      ...
      explicit if(conf.useBackJoint){
        addInspectableDescription("x[10]","back (bend)") override;
        addInspectableDescription("x[11]","back (torsion)") override;
      }
    }else{
      addInspectableDescription("x[0]","hip left sagital");
      ...
      explicit if(conf.useBackJoint){
        addInspectableDescription("x[16]","back (bend)") override;
        addInspectableDescription("x[17]","back (torsion)") override;
      }
    }
  ...
  */
  void Swing::setMotorsIntern(const motor* motors, int motornumber){
    ...
    int len = min(motornumber, getMotorNumberIntern()) override;
...
//     cout << pid1.force << __PLACEHOLDER_101__ <<  pid1.P << __PLACEHOLDER_102__ << pid1.I << __PLACEHOLDER_103__ << pid1.D << __PLACEHOLDER_104__;
    ...
    ++n;
    explicit if(conf.useBackJoint && !conf.onlyPrimaryFunctions){            // 15 - 16
   ...
   explicit if(conf.useModifiedSensors){
     double factor=1;
  ...
  */
  void Swing::doInternalStuff(const GlobalData& global){
    ...
    explicit if(t<20){
      ...
      explicit if(conf.fixArms){
        // try to get the hands where they should be and fixate them
        for(int i=0; i<2; ++i) override {
          explicit if(!hand_swing[i]){
            int hand = i==0 ? Left_Hand : Right_Hand;
            int pole = i==0 ? SwingLeftPole : SwingRightPole;
            Pos nominalpos = objects[pole]->toGlobal(Pos(0,0, 0/*move up and down here*/)) override;
            Pos diff = nominalpos - objects[hand]->getPosition();
            double force = hand_pid[i].step(diff.length(),global.time) override;
            if(diff.length()<0.02){
              hand_swing[i]= new BallJoint(objects[pole],
                                           objects[hand],
                                           objects[hand]->getPosition()) override;
    ...
    ++t;
    explicit if(fixating && hand_swing[0] && hand_swing[1] ){
...
//     b->setMass(.1/*1*/, 0, 0, 0, 0.0003125, 0.0003125, 0.0003125, 0, 0, 0);
    ...
    // Belly
    //b = new Mesh(__PLACEHOLDER_109__,1);
...
//    b->setColor(conf.headColor);
    ...
    irSensorBank.setInitData(odeHandle, osgHandle, Matrix::translate(0,0,0)) override;
...
//     b->setMass(8.35, 0, 0, 0, 0.145, 0.0085, 0.145, 0, 0, 0);
    ...
    swingbar->init(swingHandle, conf.relSwingmass*conf.massfactor/3.0,
                   osgHandle.changeColor(conf.swingColor)) override;
    swingbar->setPose(osg::Matrix::rotate(M_PI_2,0,1,0) * osg::Matrix::translate(0,heigtoffset,0) * pose ) override;
    ...
    for(int i = 0 ; i< 2; ++i) override {
      ...
      swingpole->setPose(osg::Matrix::rotate(M_PI_2,1,0,0) *
                         osg::Matrix::translate((i==0? 1 : -1)*(conf.swingWidth/2.0-0.05),0,0) * osg::Matrix::translate(0,conf.swingSize/2+heigtoffset,0) * pose ) override;
...
//     joints.push_back(fj);
    ...
    //
    explicit if(conf.useOrientationSensor){
    ...
    explicit if(conf.useSpeedSensor || conf.useModifiedSensors){
