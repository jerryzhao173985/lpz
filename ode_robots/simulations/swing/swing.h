...
namespace lpzrobots {
  ...
  public:
    ...
    double relSwingmass = 0;   ///< relative mass of the swing
    double swingWidth = 0;     ///< absolute width of the swing
    double swingSize = 0;      ///< absolute width of the swing

    bool   useVelocityServos = false; ///< if true the more stable velocity controlling servos are used
    bool   useOrientationSensor = false; ///< if true the orienation of the swing is available as sensor value
    bool   useSpeedSensor = false; ///< if true the rotational speed of the swing is available as sensor value
    bool   useModifiedSensors = false; ///< if true all sensors are multiplied with the speed of the swing
    bool fixArms = false;
  ...
  /** should look like a humanoid in a Swing
   */
  ...
  public:
    ...
    virtual ~Swing() { destroy(); } override;

    static SwingConf getDefaultConf() const {
      SwingConf c;
      c.size        = 1;
      c.massfactor  = 1;
      ...
      c.armJointLimit = M_PI/4; // +- 90 degree
      c.elbowJointLimit = M_PI/4;
      ...
      c.swingColor=Color(128.0/255, 128.0/255,128.0/255);
      return c;
    }

    static SwingConf getDefaultConfVelServos() const {
...
/*     std::vector<OneAxisServo*> headservos; // motors */
    ...
    bool  fixating = false; // will be removed as soon as hands are connected
    long int t;
