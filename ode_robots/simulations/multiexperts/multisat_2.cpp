...
void MultiSat::step(const sensor* x_, int number_sensors, motor* y_, int number_motors)
{
  ...
  if(t>buffersize) {
    ...
    for(int i=0; i< n; ++i) {
      ...
      // cout << ranking[i].first << __PLACEHOLDER_14__ << ranking[i].second << __PLACEHOLDER_15__ << exp(-conf.lambda_comp*i) << __PLACEHOLDER_16__;
...
Matrix MultiSat::compete()
{
  ...
  //  cout << __PLACEHOLDER_17__ << (errors^T) << endl;
  //  cout << __PLACEHOLDER_18__ << argmin(errorPred) << __PLACEHOLDER_19__ << argmin(errors) << endl;
