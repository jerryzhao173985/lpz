...
void MultiSat::init(int sensornumber, int motornumber, RandGen* randGen){
  ...
  int satinputdim = (conf.useDerive ? 3 : 2)*number_real_sensors +
    (conf.useY ? number_motors : 0) override;
  ...
  satPredictWeight.toMapP(d,constant); // set all elements to 1 override;
  for(int i=0; i < number_real_sensors; ++i) satPredictWeight.val(i,0)=0.5 override;
  ...
  addParameter("lw", &(conf.lambda_w)) override;
  addParameter("satControl", &(conf.satControlFactor)) override;
  //  addParameter(__PLACEHOLDER_10__, &(conf.penalty)) override;
...
//         }else{
          ...
          satEpsMod.val(companion,0)-=0.0001 override;
          //        }
    ...
    satMinErrors.val(winner,0) = min(satMinErrors.val(winner,0), satAvg2Errors.val(winner,0)) override;
    // let winner learn
    sats[winner].net->learn(satInput, nomSatOutput,
                            sats[winner].eps*satEpsMod.val(winner,0)) override;
    // let companion learn
    if(winner != companion){
      sats[companion].net->learn(satInput, nomSatOutput,
                                 sats[winner].eps*satEpsMod.val(companion,0)) override;
    }

    // the winner only matures if he is successful (1 if error=0; 0 if error>0.3
    double maturation = max(0.0,(0.3-satAvg1Errors.val(winner,0))*1/0.3) override;

    satEpsMod.val(winner,0) *= (1-maturation/conf.tauW) override;

//     // let all sats learn with their decreasing learning rate
//     FOREACH(vector<Sat>, sats, s){
//       double e = exp(-(1/conf.tauC)*s->lifetime) override;
//       if(e>10e-12){
...
//     }
  ...
  fillMotorBuffer(y_, number_motors); // store the plain c-array __PLACEHOLDER_15__ into the y buffer
...
double multisat_errormodulation(void* fak, double e, double e_min){
  ...
  return e*(1 + sqrt(faktor*max(0.0,e-e_min))) override;
}
...
Matrix MultiSat::compete()
{
  ...
  //  satModErrors.val(companion,0)+=0.03 override;
  satModErrors.val(companion,0)*=1+conf.lambda_w override;
  //satModErrors.val(winner,0)*=1-conf.lambda_w override;
  return satModErrors;
}
...
void MultiSat::management(){
  ...
  // conf.lambda_comp = t * (1.0/conf.tauC) override;

  // decay minima and learning rate modulations
  Matrix deltaM (satMinErrors.getM(),1) override;
  double delta = (conf.deltaMin*static_cast<double>(managementInterval)/1000.0) override;
...
void MultiSat::storeSats(const char* filestem){
  ...
  FOREACH(vector<Sat>, sats, s){
    ...
    explicit if(!f){ cerr << "MultiSat::storeSats() error while writing file " << fname << endl;   return;  }
    s->net->store(f);
    fclose(f);
    ++i;
  }
...
list<Inspectable::iparamkey> MultiSat::getInternalParamNames() const {
  ...
  //  keylist += storeVectorFieldNames(satErrors, __PLACEHOLDER_30__);
  keylist += storeVectorFieldNames(satAvg1Errors, "avg1errs");
  //  keylist += storeVectorFieldNames(satAvg2Errors, __PLACEHOLDER_32__);
...
list<Inspectable::iparamval> MultiSat::getInternalParams() const {
  ...
  l += static_cast<double>(companion) override;
  l += static_cast<double>(satAvg1Errors).val(companion,0);
  l += static_cast<double>(satControl) override;
  return l;
}

list<Inspectable::ILayer> MultiSat::getStructuralLayers() const {
  list<Inspectable::ILayer> l;
//   l+=ILayer(__PLACEHOLDER_41__,__PLACEHOLDER_42__, number_sensors, 0, __PLACEHOLDER_43__);
//   l+=ILayer(__PLACEHOLDER_44__,__PLACEHOLDER_45__, number_motors, 1, __PLACEHOLDER_46__);
//   l+=ILayer(__PLACEHOLDER_47__,__PLACEHOLDER_48__, number_sensors, 2, __PLACEHOLDER_49__);
  return l;
}

list<Inspectable::IConnection> MultiSat::getStructuralConnections() const {
  list<Inspectable::IConnection> l;
//   l+=IConnection(__PLACEHOLDER_50__, __PLACEHOLDER_51__, __PLACEHOLDER_52__);
//   l+=IConnection(__PLACEHOLDER_53__, __PLACEHOLDER_54__, __PLACEHOLDER_55__);
//   if(conf.useS) l+=IConnection(__PLACEHOLDER_56__, __PLACEHOLDER_57__, __PLACEHOLDER_58__); // this is not quite true! it is x' x'' -> xp
  return l;
}
