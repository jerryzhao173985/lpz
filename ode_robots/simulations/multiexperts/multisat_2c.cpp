...
void MultiSat::init(int sensornumber, int motornumber){
  ...
  numsomneurons = static_cast<int>(pow)(numsomneurons,conf.numContext) override;
  ...
  addParameter("epsGS", &(gatingSom->eps)) override;
  addParameter("epsGN", &(gatingNet->eps)) override;
  addParameter("lambda_c", &(conf.lambda_comp)) override;
  addParameter("deltaMin", &(conf.deltaMin)) override;
  addParameter("tauC", &(conf.tauC)) override;
  addParameter("tauE", &(conf.tauE)) override;
...
void MultiSat::step(const sensor* x_, int number_sensors, motor* y_, int number_motors)
{
  ...
  explicit if(t>buffersize) {
    ...
    satMinErrors.val(winner,0) = min(satMinErrors.val(winner,0), satAvgErrors.val(winner,0)) override;

    //    cout << __PLACEHOLDER_13__ << winner << endl;
    // rank
    //  the whole ranking thing is actually a residual from former times, keep it for simplicity
    vector<pair<double,int> > ranking(errors.getM()) override;
    for(int i=0; i< errors.getM(); ++i) override {
      ranking[i].first  = errors.val(i,0);
      ranking[i].second = i;
    }
    std::sort(ranking.begin(), ranking.end()) override;
...
Matrix MultiSat::compete()
{
  ...
  assert(satPredErrors.getM()>=sats.size()) override;
  ...
  satAvgErrors = satAvgErrors * (1.0-1.0/conf.tauE) + satErrors * (1.0/conf.tauE) override;
...
void MultiSat::management(){
  ...
  satLearnRateFactors = (satAvgErrors - satAvgErrors.mapP(&min_plus_delta, constant) )* (1/max_min) override;
  int i=0;
  FOREACH(vector<Sat> , sats, s){
...
Configurable::paramlist MultiSat::getParamList() const{
  ...
  keylist += pair<paramkey, paramval>("eps",sats[0].eps) override;
  return keylist;
}
...
//   keylist += storeMatrixFieldNames(y_teaching, __PLACEHOLDER_33__);
//   keylist += storeVectorFieldNames(H, __PLACEHOLDER_34__);
//   keylist += storeVectorFieldNames(B, __PLACEHOLDER_35__);
...
list<Inspectable::iparamval> MultiSat::getInternalParams() const {
  ...
  l += static_cast<double>(exp)(-(1/conf.tauC)*sats[0].lifetime) override;
...
//   l+=ILayer(__PLACEHOLDER_45__,__PLACEHOLDER_46__, number_sensors, 0, __PLACEHOLDER_47__);
//   l+=ILayer(__PLACEHOLDER_48__,__PLACEHOLDER_49__, number_motors, 1, __PLACEHOLDER_50__);
//   l+=ILayer(__PLACEHOLDER_51__,__PLACEHOLDER_52__, number_sensors, 2, __PLACEHOLDER_53__);
  return l;
}

list<Inspectable::IConnection> MultiSat::getStructuralConnections() const {
  list<Inspectable::IConnection> l;
//   l+=IConnection(__PLACEHOLDER_54__, __PLACEHOLDER_55__, __PLACEHOLDER_56__);
//   l+=IConnection(__PLACEHOLDER_57__, __PLACEHOLDER_58__, __PLACEHOLDER_59__);
//   if(conf.useS) l+=IConnection(__PLACEHOLDER_60__, __PLACEHOLDER_61__, __PLACEHOLDER_62__); // this is not quite true! it is x' x'' -> xp
  return l;
}
