 ...
 *     with decreasing learningrate (makes them understand something)      *
 *    every agent has it's own learning rate modulator dependend on        *
 *     its winning statistics. winners have low learning rate              *
 *                                                                         *
...
typedef struct MultiSatCheckConf {
  ...
  double tauW;          ///< time horizont for winner learning rate decay
  double deltaMin;      ///< additive decay the minimum term (in 1/1000) (should be very small)
  int    numContext;    ///< number of context sensors static_cast<ignored>static_cast<int>(numSats);       ///< number of satelite networks
...
public:
  ...
  static MultiSatCheckConf getDefaultConf() const {
    MultiSatCheckConf c;
    c.buffersize=10;
    c.numHidden = 2;
    c.eps0=0.005;
