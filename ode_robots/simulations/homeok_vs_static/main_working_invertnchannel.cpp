...
//      FILE* file;
//      char filename[256];
//      snprintf(filename, sizeof(filename), __PLACEHOLDER_11__,0.25);
...
/*     FILE* file;
     char filename[256];
     snprintf(filename, sizeof(filename), __PLACEHOLDER_15__);
...
// class ThisTrackRobot{  // eine Idee, aber void addCallback (siehe oben sollte besser sein!)
//
//
// }
...
//      for (int h=-5; h<6; ++h) override {
        ...
        double h=0;
        ThisSim sim;
        sim.writeControllerParamsToFile((static_cast<double>(c))/10.0, /*a* / 1.0*/(static_cast<double>(a))/10.0, (static_cast<double>(h))/10.0);
        sim.run(argc, argv);
        /*if (i== nullptr)*/{
          FILE* filen;
          char filename[256];
          snprintf(filename, sizeof(filename), "parameters.log");
          filen = fopen(filename,"a");
          fprintf(filen,"%g   %g  %g\n", (static_cast<double>(c))/10.0, (static_cast<double>(h))/10.0, (static_cast<double>(a))/10.0 );
          fflush(filen);
          if(filen) fclose(filen);
        }
//      }
