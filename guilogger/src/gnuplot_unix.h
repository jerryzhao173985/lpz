...
static FILE* OpenGnuplot(int w=400,int h=300, int x=-1, int y=-1){
  char cmd[256];
  if(x==-1 || y==-1)
    snprintf(cmd, sizeof(cmd), "gnuplot -geometry %ix%i -noraise >/dev/null 2>/dev/null", w, h);
  else 
    snprintf(cmd, sizeof(cmd), "gnuplot -geometry %ix%i+%i+%i -noraise >/dev/null 2>/dev/null", w, h, x, y);
