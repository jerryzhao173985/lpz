  ...
  */
  virtual void setMotors(const motor* motors, int motornumber) override {
    ...
    /*  simple discrete simulation
        a = F/m - \mu v_0/m __PLACEHOLDER_47__
        v = a*t + v0
        x = v*t + x0
    */
    Position a(y[0],0,0);
    ...
    if(touchs.empty()){
      ...
      x[len] = 0.9*(touchs.begin()->value) + 0.1* y[1];
...
void printRobots(const list<MyRobot*>& robots){
  ...
  FOREACHC(list<MyRobot*>, robots, i) {
    ...
    //  line[coord(x)]=__PLACEHOLDER_38__+ k;
    ++k;
  }
  k=0;
  // objects
...
int main(int argc, char** argv){
  ...
  for (int i=0; i<1; ++i) {
    ...
    //    if(i== nullptr) agent->setTrackOptions(TrackRobot(true,true,false, false,__PLACEHOLDER_30__,5));
