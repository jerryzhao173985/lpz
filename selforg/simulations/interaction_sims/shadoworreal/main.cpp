...
/*
This experiment is inspired by Marika Auvrey.
The original experiment by V. Trevarthen was originally conduced
with babies 18month  and their mothers.
The baby does not react on a replay of the mother.
Here we have two agents which can move a long a one-dimensional
space. (Eziquiel DiPaolo used this as well).
They can touch the other when they are at the some place or
...
bool stop         = 0;
double sleep_     = 1000; // 0;
double shadowdist = 0.6;
bool usestatic    = false;
bool usereinforc  = true;
double maxsimtime = 3000; // 3000;
  ...
  */
  virtual void setMotors(const motor* motors, int motornumber) override {
    ...
    /*  simple discrete simulation
        a = F/m - \mu v_0/m __PLACEHOLDER_52__
        v = a*t + v0
        x = v*t + x0
    */
    Position a(y[0],0,0);
    ...
    FOREACHC(list<const MyRobot*>, otherRobots, i){
      ...
      // static object
      Position q(0,0,0);
      double qdist = toEnv(pos.x-q.x);

      double rdist = toEnv(pos.x-opos.x);
      real_dist = rdist;
      double sdist = toEnv(pos.x-shadow.x);
      // feel shortest object (there should be only one in range anyway)
      double dist = fabs(rdist) < fabs(sdist) ? rdist : sdist;
      bool sensestat=false;
      if(usestatic && (fabs(qdist) < fabs(dist))) {
              dist = qdist;
        sensestat=true;
      }
      ...
      if(dist < -range) x[len]= 0;
        ...
        // x[len] = sensorscale * tactilerect(dist,range);
      }
      ++len;
      ++k;
      if (len>sensornumber){
  ...
  virtual Position getAngularSpeed() const override {return Position(real_dist,x[1],whatDoIFeel);}
  ...
  double tactilerect(double dist, double range){
    return 1;
  }
...
public:
  ...
  double real_dist; // distance to real other robot
};
...
int main(int argc, char** argv){
  ...
  for (int i=0; i<2; ++i) {
    ...
    if(i==0)
      //      agent->setTrackOptions(TrackRobot(true,true,false, false,__PLACEHOLDER_31__,10));
      agent->setTrackOptions(TrackRobot(true,true,false, false,"updown_static",10));
    //robot->setParam(__PLACEHOLDER_33__,0.2);
    //    robot->setParam(__PLACEHOLDER_34__,1);
