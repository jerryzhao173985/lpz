...
#define QMP_END_PARALLEL_FOR                                                                       \
  ...
  qmp_internal::ParallelTaskManager::instance().process(&QMP_UNIQUE_SYMBOL(Instance));             \
  }
...
#define QMP_ASSERT(condition)                                                                      \
  {                                                                                                \
    if (!(condition)) {                                                                            \
      std::cout << "[QuickMP] Assertion failed in " << __FUNCTION__ << "(line " << __LINE__        \
                << "): assert(" << #condition << ")" << std::endl;                                 \
      ::exit(1);                                                                                   \
    }                                                                                              \
  }
...
struct PlatformThreadObjects {
  PlatformThreadObjects()
    ...
    , barrierEvent1(nullptr)
    , barrierEvent2(nullptr)
...
#else
inline void*
threadRoutine(void* threadIndex)
#endif
{
...
void
ParallelTaskManager::setNumThreads(unsigned int numThreads) {
  if (mInitialized) {
    destroy();
  }
  ...
  if (numThreads > 1) {
    ...
    QMP_ASSERT(0 == returnCode);
    returnCode = pthread_attr_setdetachstate(&threadAttributes, PTHREAD_CREATE_JOINABLE);
    QMP_ASSERT(0 == returnCode);
...
unsigned int
ParallelTaskManager::getNumThreads() const {
...
unsigned int
ParallelTaskManager::getMaxThreads() const {
  return mNumThreads;
}

unsigned int
ParallelTaskManager::getNumProcessors() const {
#ifdef QMP_USE_WINDOWS_THREADS
...
bool
ParallelTaskManager::inParallel() const {
  return mInParallelSection;
}

void
ParallelTaskManager::setLoopIndices(int loopFirstIndex,
                                    unsigned int numIterations,
                                    quickmp::ScheduleHint scheduleHint) {
  if (!mInitialized) {
    setNumThreads();
...
void
ParallelTaskManager::setLoopIndices(int loopFirstIndex, unsigned int numIterations) {
  setLoopIndices(loopFirstIndex, numIterations, quickmp::SEQUENTIAL);
}

void
ParallelTaskManager::process(ParallelTask* task) {
...
void
ParallelTaskManager::processSubset(unsigned int threadIndex) {
  mCurrentTask->run(mTaskFirstIndices[threadIndex],
                    mTaskLastIndices[threadIndex],
                    threadIndex,
                    mTaskIndexIncrement);
}

void
ParallelTaskManager::criticalSectionBegin(unsigned int id) {
...
void
ParallelTaskManager::criticalSectionEnd(unsigned int id) {
  // Avoid synchronization if there are no worker threads.
  if (mNumThreads < 2) {
    return;
  }
...
#ifdef QMP_USE_WINDOWS_THREADS
  if (id >= mPlatform->userCriticalSections.size()) {
    std::cout << "[QuickMP] WARNING: Critical section 'end' (id=" << id
...
ParallelTaskManager::barrier() {
  ...
  if (mBarrierCount == mNumThreads) {
    ...
    returnCode = pthread_cond_wait(&mPlatform->barrierCondition, &mPlatform->barrierMutex);
    QMP_ASSERT(0 == returnCode);
    returnCode = pthread_mutex_unlock(&mPlatform->barrierMutex);
    QMP_ASSERT(0 == returnCode);
#endif
...
bool
ParallelTaskManager::shouldWorkerThreadsExit() const {
...
ParallelTaskManager::destroy() {
  ...
  if (mNumThreads > 1) {
    ...
    for (unsigned int threadIndex = 1; threadIndex < mNumThreads; ++threadIndex) {
      DWORD returnCode = WaitForSingleObject(mPlatform->threadHandles[threadIndex], INFINITE);
    ...
    // The main thread (index 0) handle is not used.
    for (unsigned int threadIndex = 1; threadIndex < mNumThreads; ++threadIndex) {
      int returnCode = CloseHandle(mPlatform->threadHandles[threadIndex]);
      QMP_ASSERT(0 != returnCode);
    }
