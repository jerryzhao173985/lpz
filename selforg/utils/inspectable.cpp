...
Inspectable::getInternalParamNames() const {
...
Inspectable::getInternalParams() const {
...
Inspectable::iparamvalptrlist
Inspectable::getInternalParamsPtr() const {
...
Inspectable::getStructuralLayers() const {
  return std::list<ILayer>();
}

Inspectable::iconnectionlist
Inspectable::getStructuralConnections() const {
  return std::list<IConnection>();
}

void
Inspectable::addInspectableValue(const iparamkey& key,
                                 iparamval const* val,
...
void
Inspectable::addInspectableMatrix(const iparamkey& key,
...
void
Inspectable::addInspectableDescription(const iparamkey& key, const std::string& descr) {
...
Inspectable::getInfoLines() const {
  return infoLineStringList;
}

void
Inspectable::removeInfoLines() {
  infoLineStringList.clear();
}

void
Inspectable::addInspectable(Inspectable* insp) {
  listOfInspectableChildren.push_back(insp);
  insp->parent = this;
}

void
Inspectable::removeInspectable(Inspectable* insp) {
  removeElement(listOfInspectableChildren, insp);
  insp->parent = 0;
}

void
Inspectable::setNameOfInspectable(const iparamkey& _name) {
  name = _name;
}

const Inspectable::iparamkey
Inspectable::getNameOfInspectable() const {
  return name;
}

const Inspectable::inspectableList&
Inspectable::getInspectables() const {
