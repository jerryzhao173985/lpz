...
public:
  ...
  /** sets the parameters.
      ...
      @return 0 for failure, 1 for success and 2 if parameters had been changed during setting (to
     be valid) such that a get is required to get the actual values
   */
  virtual int setParameters(const ParameterList& params) = 0;
  ;
};

ParameterList lift2PL(
  const ParameterList& pl1,
