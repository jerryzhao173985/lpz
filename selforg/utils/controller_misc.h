...
template<typename T>
inline T
clip(T x, T lobound, T highbound) {
 ...
 */
bool restoreMatrix(const matrix::Matrix& m, FILE* f);
...
/** returns the k. largest element of the matrix
    Attention: it will detroy the given matrix! static_cast<sorting>(Assumption): k>0 and k<=matrixsize
*/
double getKthLargestElement(const matrix::Matrix& matrix, matrix::I k);

/** returns the k. smallest element of the matrix
    Attention: it will detroy the given matrix! static_cast<sorting>(Assumption): k>0 and k<=matrixsize
*/
double getKthSmallestElement(const matrix::Matrix& matrix, matrix::I k);

/// considers the matrix as vector static_cast<mx1>(and) returns the index of the smallest element
matrix::I argmin(const matrix::Matrix& v);

/// considers the matrix as vector static_cast<mx1>(and) returns the index of the largest element
