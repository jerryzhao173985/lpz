...
bool
eigenValuesVectorsRealSym(const Matrix& m, Matrix& eigenvalues, Matrix& eigenvectors) {
  // check if m is square
  assert(m.getM() == m.getN());
  gsl_matrix* m_gsl = toGSL(m);
  gsl_set_error_handler_off();
...
bool
eigenValues(const Matrix& m, Matrix& real, Matrix& imag) {
  // check if m is square
  assert(m.getM() == m.getN());
  gsl_matrix* m_gsl = toGSL(m);
  gsl_set_error_handler_off();
...
bool
eigenValuesVectors(const Matrix& m,
                   Matrix& vals_real,
                   Matrix& vals_imag,
                   Matrix& vecs_real,
                   Matrix& vecs_imag) {
...
toPositiveSignEigenVectors(const Matrix& vecs_real, const Matrix& vecs_imag) {
  ...
  for (unsigned int i = 0; i < vecs_real.getN(); ++i) {
    ...
    if (vecs_real.val(k, i) < 0) {
      signs[i] = -1;
      // Note: The original code tried to modify const matrices here
      // If the caller needs modified matrices, they should apply the signs themselves
    } else {
      signs[i] = 1;
    }
  }
...
double
__matutils_euklidlen(double a, double b) {
  return sqrt(a * a + b * b);
}

Matrix
scaleEigenVectorsWithValue(const Matrix& vals_real,
