...
// bcmp is no longer needed - using std::memcmp instead

bool
Matrix::equals(const Matrix& a) const {
  if (m * n != a.m * a.n)
    return false;
  return (std::memcmp(data, a.data, sizeof(double) * m * n) == 0);
}
...
Matrix::rows(I startindex, I endindex) const {
  ...
  I end = static_cast<I>(std::max(static_cast<int>(start),
                                  std::min(static_cast<int>(endindex), static_cast<int>(m) - 1)));
  I k = end - start + 1;
  if (k == m)
    return *this;
  Matrix result(k, n);
...
void
Matrix::set(I _m, I _n, const D* _data /*=0*/) {
  m = _m;
  n = _n;
  allocate();
  set(_data);
...
int
Matrix::convertToBuffer(D* buffer, I len) const {
  if (buffer && data) {
...
bool
Matrix::read(FILE* f, bool skipIdentifier) {
  char buffer[128];
  if (!skipIdentifier) {
 ...
 */
bool
Matrix::restore(FILE* f) {
  char buffer[7];
  bool rval = false;
  if (fread(buffer, 7, 1, f) == 1) {
    if (buffer[0] == 'M' && buffer[1] == 'A' && buffer[2] == 'T' && buffer[3] == 'R' &&
        buffer[4] == 'I' && buffer[5] == 'X') {
      return read(f, true);
    } else {
      fseek(f, -7, SEEK_CUR);
      I dim[2];
      if (fread(dim, sizeof(I), 2, f) == 2) {
        ...
        if (fread(data, sizeof(D), len, f) == len) {
          ...
          fprintf(stderr, "Matrix::restore: static_cast<binary>(cannot) read matrix data\n");
      } else {
...
void
Matrix::add(const Matrix& a, const D& summand) {
  m = a.m;
  n = a.n;
  allocate();
  for (I i = 0; i < m * n; ++i) {
...
void
Matrix::mult(const Matrix& a, const D& fac) {
  m = a.m;
  n = a.n;
  allocate();

#ifdef __ARM_NEON
  // Use NEON optimized scalar multiplication for ARM64
  MatrixNEON::mult_scalar_neon(a, fac, *this);
#else
  // Standard scalar multiplication
  for (I i = 0; i < m * n; ++i) {
    data[i] = a.data[i] * fac;
 ...
 */
void
Matrix::exp(const Matrix& a, int exponent) {
  set(a.m, a.n, a.data);
  toExp(exponent);
}
...
Matrix
Matrix::pseudoInverse(const D& lambda) const {
  Matrix R;
  if (m > n)
    R = this->multTM();
  else
...
Matrix
Matrix::map2(D (*fun)(D, D), const Matrix& a, const Matrix& b) {
  Matrix result(a);
  result.toMap2(fun, b);
  return result;
}
...
Matrix
Matrix::map2P(D param, D (*fun)(D, D, D), const Matrix& a, const Matrix& b) {
  Matrix result(a);
...
Matrix
Matrix::map2P(void* param, D (*fun)(void*, D, D), const Matrix& a, const Matrix& b) {
  Matrix result(a);
  result.toMap2P(param, fun, b);
  return result;
}
...
cmpdouble(const void* a, const void* b) {
  return *(static_cast<const double*>(a)) < *(static_cast<const double*>(b))
           ? -1
           : (*(static_cast<const double*>(a)) > *(static_cast<const double*>(b)) ? 1 : 0);
}
...
Matrix
Matrix::above(const Matrix& b) const {
  Matrix r(*this);
  r.toAbove(b);
  return r;
}
...
Matrix
Matrix::beside(const Matrix& b) const {
  Matrix r(*this);
  r.toBeside(b);
  return r;
}
...
std::ostream&
operator<<(std::ostream& str, const Matrix& mat) {
  if (mat.m == 0 || mat.n == 0)
    return str << "0";
  else {
    //      str << mat.m << __PLACEHOLDER_17__ << mat.n << __PLACEHOLDER_18__;
    for (I i = 0; i < mat.m; ++i) {
      for (I j = 0; j < mat.n; ++j) {
        str << mat.val(i, j) << " \t";
      }
