...
void
DerController::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  DD.toId();
  DD *= 0.1; // TODO hier noise strength
  ...
  if (conf.useS)
    S.toId(); // set S to identity matrix;
...
void
DerController::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
DerController::fillBuffersAndControl(const sensor* x_,
                                     int number_sensors,
                                     motor* y_,
                                     int number_motors) {
...
void
DerController::calcEtaAndBufferIt(int delay) {
...
void
DerController::calcXsi(int delay) {
...
Matrix
DerController::model(const Matrix& x, const Matrix& y) {
  if (conf.useS) {
    return ((A * y + S * x) + B);
  } else {
...
void
DerController::calcCandHandAUpdates(Matrix& C_update,
                                    Matrix& H_update,
                                    Matrix& A_update,
                                    int y_delay) {
  ...
  for (unsigned int s = 1; s <= steps; ++s) {
    ...
    const Matrix& z = (C * x + H) * .5 + (R * y + H) * .5; // TEST: gleiche St�rke innen und aussen
    ...
    if (s == 1) {
      if (AAT.isNulltimesNull() || ((t % 10) == 5))
        const Matrix AAT_local = (A.multTM() + SmallID) ^ -1;
...
double
DerController::calcErrorFactor(const Matrix& e, bool loga, bool root) {
  double error_factor = 1;

  if (loga) { // using logarithmic error E=ln(v^T*v)
    error_factor =
      1 / (e.multTM().val(0, 0) + 0.000001) * 0.01; // factor 1/100 for normalising (empirically)
  }
  if (root) { // using root error E=(v^T*v)^(1/2)
    error_factor =
      1 / sqrt(e.multTM().val(0, 0) + 0.000001) * 0.1; // factor 1/10 for normalising (empirically)
  }
  return error_factor;
}

void
DerController::updateCandHandA(const Matrix& C_update,
                               const Matrix& H_update,
                               const Matrix& A_update,
                               double _squashSize) {
  ...
  double H_squashSize = _squashSize * 5.0;   // TEST: H soll sich schnelleer bewegen k�nnen.
...
Matrix
DerController::calculateControllerValues(const Matrix& x_smooth) {
  return (C * x_smooth + H).map(g);
};

void
DerController::getLastMotors(motor* motors, int len) {
  const Matrix& y = y_buffer[t % buffersize];
  y.convertToBuffer(motors, len);
}

bool
DerController::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  A.store(f);
...
DerController::getInternalParamNames() const {
...
DerController::getInternalParams() const {
  list<iparamval> l;
  if (conf.someInternalParams) {
...
DerController::getStructuralLayers() const {
...
DerController::getStructuralConnections() const {
...
void
DerController::setTeachingMode(bool onOff) {
...
void
DerController::setMotorTeachingSignal(const motor* teaching, int len) {
  assert(len == number_motors);
  y_teaching.set(len, 1, teaching);
}

double
DerController::calcMatrixNorm(const Matrix& m) {
