...
MotorBabbler::MotorBabbler(function func)
  : AbstractController("motorbabbler", "0.2")
  , randGen(0) {
  ...
  addParameterDef(
    "resampling", &resampling, 500, 1, 2000, "resample frequencies every so often (steps)");
...
void
MotorBabbler::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
MotorBabbler::sampleFrequencies() {
...
void
MotorBabbler::stepNoLearning(const sensor* sensors,
                             int number_sensors,
                             motor* motors,
                             int number_motors) {
  if (t >= resampling) {
...
double
MotorBabbler::sine(double x) {
  return sin(x);
}

double
MotorBabbler::sawtooth(double x) {
  while (x > M_PI)
    x -= 2 * M_PI;
  while (x < -M_PI)
    x += 2 * M_PI;
