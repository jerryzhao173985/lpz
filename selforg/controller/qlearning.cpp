...
QLearning::QLearning(double eps,
                     double discount,
                     double exploration,
                     int eligibility,
                     bool random_initQ,
...
void
QLearning::init(unsigned int stateDim, unsigned int actionDim, RandGen* randGen) {
  if (!randGen)
...
unsigned int
QLearning::select(unsigned int state) {
  assert(initialised);
  assert(state < Q.getM());
...
unsigned int
QLearning::select_sample(unsigned int state) {
  assert(initialised);
  assert(state < Q.getM());
...
unsigned int
QLearning::select_keepold(unsigned int state) {
  assert(initialised);
  assert(state < Q.getM());
...
matrix::Matrix
QLearning::getActionValues(unsigned int state) {
  return Q.row(state);
}

double
QLearning::learn(unsigned int state, unsigned int action, double reward, double learnRateFactor) {
  assert(initialised);
...
unsigned int
QLearning::getStateDim() const {
  return Q.getM();
}

unsigned int
QLearning::getActionDim() const {
  return Q.getN();
}

/// returns the collectedReward reward
double
QLearning::getCollectedReward() const {
  return collectedReward / (std::min(t, tau));
}

int
QLearning::valInCrossProd(const std::list<std::pair<int, int>>& vals) {
  int fac = 1;
  int val = 0;
...
std::list<int>
QLearning::ConfInCrossProd(const std::list<int>& ranges, int val) {
  std::list<int> cfg;
