...
void
Elman::init(unsigned int inputDim, unsigned int outputDim, double unit_map, RandGen* randGen) {
...
const Matrix
Elman::process(const Matrix& input) {
  assert(initialised);
...
const Matrix
Elman::learn(const Matrix& input, const Matrix& nom_output, double learnRateFactor) {
  assert(initialised);
...
Elman::weightIncrement(const matrix::Matrix& xsi_) {
  ...
  assert(weights.size() == static_cast<unsigned>(layernum));
  assert(bias.size() == static_cast<unsigned>(layernum));
  NetUpdate update(layernum, layernum, (useBypass ? 1 : 0) + (useElman ? 1 : 0) + (useJordan ? 1 : 0));
  int k = 0;
  // calculate weight updates
  Matrix delta;
  for(int i = layernum - 1; i >= 0; --i) {
...
NetUpdate
Elman::weightIncrementBlocked(const matrix::Matrix& xsi_,
                              int blockedlayer,
                              int blockfrom,
                              int blockto) {
  assert(initialised);
...
Elman::updateWeights(const NetUpdate& update) {
  ...
  assert(static_cast<int>(update.weights.size()) == layernum &&
         static_cast<int>(update.other.size()) == (useBypass ? 1 : 0) + (useElman ? 1 : 0) + (useJordan ? 1 : 0));
...
bool
Elman::restore(FILE* f) {
  bool rv = MultiLayerFFNN::restore(f);
  if (!rv)
    return false;
  if (fread(&useElman, sizeof(bool), 1, f) != 1)
    return false;
  if (useElman) {
    elmanWeights.restore(f);
    elmanContext.restore(f);
  }
  if (fread(&useJordan, sizeof(bool), 1, f) != 1)
    return false;
  if (useJordan) {
...
Elman::getInternalParamNames() const {
  if (useElman || useJordan) {
    list<iparamkey> keylist = MultiLayerFFNN::getInternalParamNames();
    if (someInternalParams) {
      if (useElman)
        keylist += store4x4AndDiagonalFieldNames(elmanWeights, "El");
      if (useJordan)
        keylist += store4x4AndDiagonalFieldNames(jordanWeights, "Jo");
    } else {
      if (useElman)
        keylist += storeMatrixFieldNames(elmanWeights, "El");
      if (useJordan)
        keylist += storeMatrixFieldNames(jordanWeights, "Jo");
    }
    return keylist;
  } else
    return MultiLayerFFNN::getInternalParamNames();
}

Inspectable::iparamvallist
Elman::getInternalParams() const {
  if (useElman || useJordan) {
    list<iparamval> l = MultiLayerFFNN::getInternalParams();
    if (someInternalParams) {
      if (useElman)
        l += store4x4AndDiagonal(elmanWeights);
      if (useJordan)
        l += store4x4AndDiagonal(jordanWeights);
    } else {
      if (useElman)
        l += elmanWeights.convertToList();
      if (useJordan)
        l += elmanWeights.convertToList();
    }
    return l;
  } else
    return MultiLayerFFNN::getInternalParams();
}

Inspectable::ilayerlist
Elman::getStructuralLayers() const {
...
Elman::getStructuralConnections() const {
