...
DerLinUnivers::DerLinUnivers(const DerLinUniversConf& conf)
  : AbstractController("derlinunivers", "$Id$")
  , t(0)
  , number_sensors(0)
  , number_motors(0)
  , initialised(false)
...
void
DerLinUnivers::init(int sensornumber, int motornumber, RandGen* randGen) {
  number_sensors = sensornumber;
  number_motors = motornumber;
  // Note: randGen parameter is not used in this implementation
  assert(conf.net);
...
void
DerLinUnivers::step(const sensor* x_, int sensornumber, motor* y_, int motornumber) {
  ...
  if (t > conf.buffersize) {
    ...
    // Now we calculate the  update of the controller prameters with liner activation functions of
    // the neural network.
    ...
    NetUpdate cupdate2 = conf.net->weightIncrementBlocked(xsi, conf.motorlayer, number_motors, -1);
    conf.net->process(x_tm1); //  activate again with original input!
    NetUpdate cupdate1 = conf.net->weightIncrementBlocked(xsi, conf.motorlayer, number_motors, -1);
    // set actfun back
    conf.net->setActivationFunctions(oldactfunlist);

    // Now we have to do the gradient descent step on the error caused by the nonlinearities of the
    // true activeation funcitons:
...
void
DerLinUnivers::fillBuffersAndControl(const sensor* x_,
                                     int number_sensors,
                                     motor* y_,
                                     int number_motors) {
...
Matrix
DerLinUnivers::calculateControllerValues(const matrix::Matrix& x) {
  /*const Matrix& xp_tp1 = */
  conf.net->process(x);
...
double
DerLinUnivers::calcErrorFactor(const matrix::Matrix& e, int Enorm) {
...
bool
DerLinUnivers::store(FILE* f) const {
  if (conf.net->store(f)) {
...
DerLinUnivers::getInternalParamNames() const {
...
DerLinUnivers::getInternalParams() const {
...
DerLinUnivers::getStructuralLayers() const {
  return conf.net->getStructuralLayers();
}

Inspectable::iconnectionlist
DerLinUnivers::getStructuralConnections() const {
