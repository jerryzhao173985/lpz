...
protected:
  ...
  int number_steps_for_averaging = 0;    ///< number of steps for averaging when calculating x_effective and y_effective
  int t = 0;       ///< number of steps, needed for ringbuffer x_buffer
  double m = 0.0;  ///< factor between E and E_s
  double a_factor = 0.0;  ///< additional factor for learning rate of model parameters
...
public:
  ...
  RobotLearnControl(double eps=0.7,double rho=0.0, int number_steps_of_delay_=3):
   : eps(eps), rho(rho), number_steps_of_delay(number_steps_of_delay_), delta(0.01), number_steps_for_averaging(1), t(0), m(0.0), a_factor (1.0), h(0) {
  ...
  virtual void explicit setAFactor(double x) {
    a_factor = x;
  };
