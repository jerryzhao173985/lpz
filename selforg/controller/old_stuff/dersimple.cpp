...
void DerSimple::learnModel(int delay){
...
void DerSimple::getLastMotors(motor* motors, int len){
...
void DerSimple::notifyOnChange(const paramkey& key){
  if(key == "epsA") {
...
list<Inspectable::ILayer> DerSimple::getStructuralLayers() const {
...
list<Inspectable::IConnection> DerSimple::getStructuralConnections() const {
...
double DerSimple::calcMatrixNorm(const Matrix& m){
