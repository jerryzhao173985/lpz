...
void
DerLinInvert::init(int sensornumber, int motornumber, RandGen* randg) {
...
void
DerLinInvert::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
DerLinInvert::fillBuffersAndControl(const sensor* x_,
                                    int number_sensors,
                                    motor* y_,
                                    int number_motors) {
  ...
  //     ((y_smooth + (y_integration) - PID_deriv*PIDdrv ).map(g)).convertToBuffer(y_,
  //     number_motors);
...
*/
void
DerLinInvert::learnController(int delay) {
  ...
  //  const Matrix& z          = (C * x/*_smooth*/ + C * H)* intstate + y_smooth*(1-intstate);//TEST
  //  innerer Zustand
  ...
  if (!conf.useS) {
    ...
    eta = eta.map(g);
    //    //  xsi = x_smooth  -  x_smooth_long -  A* ( y * weighting + y_sat * (1 - weighting ));//
    //    - v_smooth;
    A += xsi * (y ^ T) * epsA;
    ...
    //  const Matrix Evau =
    //  ((((((vau.multrowwise(vau)).map(one_over))*1).map(g))*0.0).toSum(1.0))*EE;//.map(sqrt);
    ...
    if (TLE > -1) /*< EE_mean + sqrt(EE_sqr)*.3)*/ {
      // TEST mue
      const Matrix mue1_local = xsi;

      C_update = (((mue1_local * ((/*(x_delay + H +*/ vau) ^ T)) +
                   ((mue.multrowwise(y)).multrowwise(zeta) * (-2) * ((x_delay + H) ^ T))) *
                  EE)
                     .map(g) *
                   epsC * gamma +
                 DeltaC_Old * (1 - gamma);

      HY_update = ((((mue1_local.multrowwise(y)).multrowwise(zeta) * (-2)) +
    ...
    { // Lernen nur wenn Modell einigermaen gut!!
      C_update = (((kappa * ((/*(x_delay + H) +*/ vau) ^ T) * .8 /*TEST*/ +
                    (kappag.multrowwise(Cvau) * (-2)) * ((x_delay + H) ^ T)) *
                   EE)
                      .map(g) *
                    epsC * gamma +
                  DeltaC_Old * (1 - gamma));
      //         C += (( kappa * (((x_smooth_long + H) +  vau)^T)*.4/*TEST*/ + ( kappag.multrowwise(
      //         Cvau)*(-2))* ((x_delay + H)^T) )*EE).map(g)*epsC;
...
void
DerLinInvert::learnModel(int delay) {
...
DerLinInvert::calculateControllerValues(const Matrix& x) {
  ...
  //     return ySat; }
  //   else {
  //  return ((C* x_smooth+H).map(g)) * weighting + ( conf.sat->process(x_smooth) * ( 1.0 -
  //  weighting));
  //   return (C* ( x +   H ) + HY).map(g)  * weighting + ( y_sat * ( 1.0 - weighting));
  return ((C * (x + H) + HY)
            .map(g)); // + (A0^(-1))*B*(-1.0))  * weighting + ( y_sat * ( 1.0 - weighting));;
                      // return ((C* ( x +   H )) *intstate  + y_smooth*(1-intstate) ).map(g)  *
                      // weighting + ( y_sat * ( 1.0 - weighting));//TEST Innerer Zustand
                      // }
...
void
DerLinInvert::getLastMotors(motor* motors, int len) {
...
bool
DerLinInvert::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  HY.store(f);
...
DerLinInvert::getInternalParamNames() const {
...
DerLinInvert::getInternalParams() const {
  list<iparamval> l;
  if (conf.someInternalParams) {
...
DerLinInvert::getStructuralLayers() const {
...
DerLinInvert::getStructuralConnections() const {
...
void
DerLinInvert::setMotorTeachingSignal(const motor* teaching, int len) {
...
void
DerLinInvert::setSensorTeachingSignal(const sensor* teaching, int len) {
...
double
DerLinInvert::calcMatrixNorm(const Matrix& m) {
