...
void
UniversalController::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
UniversalController::step(const sensor* x_, int sensornumber, motor* y_, int motornumber) {
  ...
  if (t > conf.buffersize) {
    ...
    // updateing the input shift v
    Matrix xsi;
    //     for (int k=0; k<5; ++k) {
    //       const Matrix& xp  = conf.net->process(x_tm1 + v);
    //       // const Matrix& xp  = conf.net->process(x_tm1); // for direct v
    //       xsi = x  - xp;
    //       J   = conf.net->response(Matrix());
    //       // the iterative calculation of v has the disadvantage,
    //       //   that it is small in the saturation
    //       v +=  (J^T)*(xsi*epsV) - v*lambda; // iterative v calculation
    ...
    conf.net->process(x_tm1 + v); //  activate again with new shifted input (maybe not necessary)
    xsi = x - conf.net->process(x_tm1); // calc xsi with original input
    // calculate model update
    NetUpdate modelupdate =
      conf.net->weightIncrementBlocked(xsi, conf.motorlayer, 0, number_motors);
    // calculate update with different dactfun
    std::vector<ActivationFunction> oldactfunlist =
      conf.net->setActivationFunction(FeedForwardNN::linear);
    // calculation step
    ...
    NetUpdate cupdate2 =
      conf.net->weightIncrementBlocked(xsi_smooth, conf.motorlayer, number_motors, -1);
...
void
UniversalController::stepNoLearning(const sensor* x,
                                    int number_sensors,
                                    motor* y,
                                    int number_motors) {
...
void
UniversalController::fillBuffersAndControl(const sensor* x_,
                                           int number_sensors,
                                           motor* y_,
                                           int number_motors) {
...
Matrix
UniversalController::calculateControllerValues(const matrix::Matrix& x) {
  /*const Matrix& xp_tp1 = */
  conf.net->process(x);
...
double
UniversalController::calcErrorFactor(const matrix::Matrix& e, int Enorm) {
...
bool
UniversalController::store(FILE* f) const {
  if (conf.net->store(f)) {
...
bool
UniversalController::restore(FILE* f) {
...
UniversalController::getInternalParamNames() const {
...
UniversalController::getInternalParams() const {
...
UniversalController::getStructuralLayers() const {
  return conf.net->getStructuralLayers();
}

Inspectable::iconnectionlist
UniversalController::getStructuralConnections() const {
