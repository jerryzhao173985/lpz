...
void UniversalController::init(int sensornumber, int motornumber, RandGen* randGen){
...
void UniversalController::step(const sensor* x_, int sensornumber,
  ...
  if(t > conf.buffersize){
    ...
    NetUpdate cupdate2 = conf.net->weightIncrementBlocked(xsi_smooth, conf.motorlayer,
                                                          number_motors, -1);
    conf.net->process(x_tm1); //  activate again with original input!
    NetUpdate cupdate1 = conf.net->weightIncrementBlocked(xsi_smooth, conf.motorlayer,
                                                          number_motors, -1);
...
void UniversalController::stepNoLearning(const sensor* x, int number_sensors,
...
void UniversalController::fillBuffersAndControl(const sensor* x_, int number_sensors,
...
Matrix UniversalController::calculateControllerValues(const matrix::Matrix& x){
  /*const Matrix& xp_tp1 = */
  conf.net->process(x);
...
Matrix UniversalController::calculateSmoothValues(const Matrix* buffer,
...
double UniversalController::calcErrorFactor(const matrix::Matrix& e, int Enorm){
...
bool UniversalController::store(FILE* f) const {
  if(conf.net->store(f)){
...
bool UniversalController::restore(FILE* f) {
...
std::list<Inspectable::iparamkey> UniversalController::getInternalParamNames() const  {
...
std::list<Inspectable::iparamval> UniversalController::getInternalParams() const {
...
Inspectable::ilayerlist UniversalController::getStructuralLayers() const{
  return conf.net->getStructuralLayers();
}

Inspectable::iconnectionlist UniversalController::getStructuralConnections() const{
