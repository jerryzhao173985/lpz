...
void
OneLayerFFNN::init(unsigned int inputDim,
                   unsigned int outputDim,
                   double unit_map,
                   RandGen* randGen) {
  if (!randGen)
    randGen = new RandGen(); // this give a memory leak!
...
const Matrix
OneLayerFFNN::process(const Matrix& input) {
  assert(initialised);
  return (weights * input + bias).map(actfun);
}
/// performs learning and returns the network output before learning
const Matrix
OneLayerFFNN::learn(const Matrix& input, const Matrix& nom_output, double learnRateFactor) {
  assert(initialised);
...
bool
OneLayerFFNN::store(FILE* f) const {
  fprintf(f, "%g\n", eps);
  weights.store(f);
  bias.store(f);
  return true;
...
bool
OneLayerFFNN::restore(FILE* f) {
  char buffer[128];
  if (fscanf(f, "%127s\n", buffer) != 1)
    return false;
  eps = atof(buffer);
  if (!weights.restore(f))
    return false;
  if (!bias.restore(f))
    return false;
  initialised = true;
