...
DEP::DEP(const DEPConf& conf)
  ...
  addParameterDef("learningrule",
                  reinterpret_cast<int*>(&this->conf.learningRule),
                  false,
                  std::string("which learning rule to use: ") +
                    std::accumulate(conf.LearningRuleNames.begin(),
                                    conf.LearningRuleNames.end(),
                                    std::string(),
  ...
  if (conf.calcEigenvalues) {
    ...
    addParameterDef(
      "evinterval", &calcEVInterval, 1, 0, 1000, "interval to update eigenvalues etc (0: never) ");
  }
  ...
  _internWithLearning =
    false; // used in step to enable learning in stepNoLearning and have only one function
};
...
void
DEP::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
DEP::motorBabblingStep(const sensor* x_,
