 ...
 */
use_java_controller::use_java_controller(const char* port_controller,
                                         const char* port_internalParams,
                                         const char* name)
  ...
  if (::bind(server_controller,
             reinterpret_cast<struct sockaddr*>(&server_controller_addr),
             sizeof(server_controller_addr)) == -1) {
    perror("bind() failed");
    exit(1);
  }
  if (::bind(server_internalParams,
             reinterpret_cast<struct sockaddr*>(&server_internalParams_addr),
             sizeof(server_internalParams_addr)) == -1) {
    perror("bind() failed");
    exit(1);
  }
  ...
  client_controller = accept(server_controller,
                             reinterpret_cast<struct sockaddr*>(&client_controller_addr),
                             &client_controller_size);

  if (client_controller == -1) {
    perror("accept() failed");
  }
  printf("ControllerServer gestartet (%s: %s)\nStatus: listen ...\n\n",
         inet_ntoa(client_controller_addr.sin_addr),
         port_controller);

  // GuiloggerServer wartet auf den GuiloggerClient (Java)
  client_internalParams_size = sizeof(client_internalParams_addr);
  client_internalParams = accept(server_internalParams,
                                 reinterpret_cast<struct sockaddr*>(&client_internalParams_addr),
                                 &client_internalParams_size);
  if (client_internalParams == -1) {
    perror("accept() failed");
  }
 ...
 */
void
use_java_controller::sendToJava(const char* message, bool abbruch, const char* meldung) {
 ...
 */
void
use_java_controller::closeJavaController() {
 ...
 */
void
use_java_controller::init(int sensornumber, int motornumber, RandGen* randgen) {
  ...
  while (1) {
    ...
    int bytes1 =
      recv(client_internalParams, recvData_internalParams, sizeof(recvData_internalParams) - 1, 0);

    recvData_internalParams[bytes1] = '\0';

    if (bytes1 > 0 && strlen(recvData_internalParams) != 0 && recvData_internalParams[0] == 'G' &&
        recvData_internalParams[1] == 'I') {
...
*/
void
use_java_controller::step(const sensor* sensors, int sensornumber, motor* motors, int motornumber) {
...
*/
void
use_java_controller::stepNoLearning(const sensor* sensors,
                                    int number_sensors,
                                    motor* motors,
                                    int number_motors) {
...
/**
 * executed when a parameter is asked from the console (value should be sent from java-controller
 * and returned)
 */
Configurable::paramval
use_java_controller::getParam(const paramkey& key) const {
 ...
 */
bool
use_java_controller::setParam(const paramkey& key, paramval val) {
  // wenn server läuft
  if (serverOK) {
...
/**
 * executed once from simulation for asking the parameter names (parameter names should be sent from
 * java-controller and returned)
 */
Configurable::paramlist
use_java_controller::getParamList() const {
