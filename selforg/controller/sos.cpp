...
void
Sos::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
Sos::setA(const matrix::Matrix& _A) {
...
void
Sos::setC(const matrix::Matrix& _C) {
...
void
Sos::seth(const matrix::Matrix& _h) {
...
void
Sos::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
Sos::learn() {
  ...
  const Matrix& xsi = x_fut - (A * y_delayed + b);

  A += (xsi * (y_delayed ^ T) * epsA + (A * -0.003) * (epsA > 0 ? 1 : 0)).mapP(0.1, clip);
