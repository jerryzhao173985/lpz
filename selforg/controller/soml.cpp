...
SoML::SoML(const SoMLConf& conf)
  : AbstractController("SoML", "0.1")
  , number_sensors(0)
  , number_motors(0)
  , cNet(nullptr)
  , numControllerLayer(0)
  , conf(conf)
  , t(0)
  , E(0) {
  ...
  addParameterDef(
    "s4delay", &s4delay, 1, 0, 20, "number of steps to delay motor values (delay in the loop)");
  addParameterDef("dampA", &dampA, 0.00001, 0, 1, "damping factor for model learning");
  if (conf.useS)
    addParameterDef("discountS", &discountS, 0.005, 0, 1, "discounting od the S term");
...
void
SoML::init(int sensornumber, int motornumber, RandGen* randGen) {
  if (!randGen)
  ...
  if (conf.useHiddenContr) {
    layers.push_back(Layer(
      static_cast<int>(number_motors) * conf.hiddenContrUnitsRatio, 1.0, FeedForwardNN::tanhr)); // hidden layer
    ++numControllerLayer;
  }
  ...
  if (conf.useHiddenModel)
    layers.push_back(Layer(
      static_cast<int>(number_motors) * conf.hiddenModelUnitsRatio, 1, FeedForwardNN::tanhr)); // hidden layer
  ...
  for (unsigned int l = 0; l < numControllerLayer; ++l) {
    addInspectableMatrix(
      "C" + itos(l + 1), &(cNet->getWeights(l)), sIP, "controller matrix of layer " + itos(l));
    addInspectableMatrix(
      "h" + itos(l + 1), &(cNet->getBias(l)), sIP, "controller bias of layer " + itos(l));
  }
  for (unsigned int l = numControllerLayer; l < cNet->getLayerNum(); ++l) {
    int i = l - numControllerLayer;
    addInspectableMatrix(
      "A" + itos(i + 1), &(cNet->getWeights(l)), sIP, "model matrix of layer " + itos(i));
    addInspectableMatrix(
      "b" + itos(i + 1), &(cNet->getBias(l)), sIP, "model bias of layer " + itos(i));
  }
  if (conf.useS) {
    addInspectableMatrix("S", &(cNet->getByPass()), sIP, "S (additional model matrix");
  }
  addInspectableMatrix("L", &cNet->response(), sIP, "Jacobian of sensorimotor loop");
  addInspectableMatrix(
    "R", &cNet->responseLinear(), sIP, "Linearized Jacobian of sensorimotor loop");
  ...
  //  addInspectableMatrix(__PLACEHOLDER_40__, &eta_avg, sIP, __PLACEHOLDER_41__);
}

/// performs one step (includes learning). Calulates motor commands from sensor inputs.
void
SoML::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
SoML::stepNoLearning(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
SoML::control(const Matrix& x, motor* y_, int number_motors) {
...
// learn cNet
void
SoML::learn(const Matrix& x, const Matrix& y) {
  ...
  for (unsigned int l = 0; l < numControllerLayer; ++l) {
    ...
    const Matrix& y_layer = cNet->getLayerOutput(l);
...
void
SoML::learnModelBP(double factor) {
  ...
  if (conf.useS) {
    ...
    S += ((delta[cNet->getLayerNum() - 1] * (x ^ T) - S * discountS) * (epsA * factor))
           .mapP(0.03, clip);
  }
}

void
SoML::motorBabblingStep(const sensor* x_, int number_sensors, const motor* y_, int number_motors) {
  assert(static_cast<unsigned>(number_sensors) <= this->number_sensors &&
         static_cast<unsigned>(number_motors) <= this->number_motors);
  ...
  const Matrix xp =
    cNet->processX(x_tm1, y_tm1, numControllerLayer - 1); // process from controller layer of
  learnModelBP(factor);
