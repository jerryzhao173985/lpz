...
SoML::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  if (conf.useHiddenContr) {
    layers.push_back(Layer(
      (int)number_motors * conf.hiddenContrUnitsRatio, 1.0, FeedForwardNN::tanhr)); // hidden layer
    numControllerLayer++;
  }
  ...
  if (conf.useHiddenModel)
    layers.push_back(Layer(
      (int)number_motors * conf.hiddenModelUnitsRatio, 1, FeedForwardNN::tanhr)); // hidden layer
...
Discount stuff:
  ...
  // else
    ...
    //   cNet->backpropagationX(xi - (cNet->response()- cNet->getByPass())*(x_tm1*discountS), 0,
&delta)
