...
FFNNController::FFNNController(const std::string& networkfilename,
                               int history,
                               bool input_only_x,
                               unsigned int init_wait)
  : AbstractController("FFNNController",
                       "$Id: ffnncontroller.cpp,v 1.10 2011/05/30 13:52:54 martius Exp $")
  , number_motors(0)
  , number_sensors(0)
  , history(history)
  , buffersize(history + 10)
  , input_only_x(input_only_x)
  , s4avg(1)
  , t(0)
  , init_wait(init_wait)
  , x_buffer(nullptr)
...
FFNNController::FFNNController(MultiLayerFFNN* net,
                               int history,
                               bool input_only_x,
                               unsigned int init_wait)
  : AbstractController("FFNNController",
...
void
FFNNController::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
FFNNController::step(const sensor* sensors, int number_sensors, motor* motors, int number_motors) {
...
matrix::Matrix
FFNNController::assembleNetworkOutput(const matrix::Matrix& output) const {
  return output;
}

bool
FFNNController::store(FILE* f) const {
  return net->store(f);
}

bool
FFNNController::restore(FILE* f) {
  return net->restore(f);
}

void
FFNNController::notifyOnChange(const paramkey& key) {
  s4avg = max(1, s4avg);
}
