...
void
ControllerNet::init(unsigned int inputDim,
...
const Matrix
ControllerNet::process(const Matrix& input) {
  assert(initialised);
  unsigned int layernum = layers.size();
...
const Matrix
ControllerNet::processX(const Matrix& input, const Matrix& injection, unsigned int injectInLayer) {
  assert(initialised);
  unsigned int layernum = layers.size();
...
const Matrix
ControllerNet::forwardpropagation(const matrix::Matrix& error,
                                  Matrices* errors,
                                  Matrices* zetas) const {
  assert(initialised);
  unsigned int layernum = layers.size();
  ...
  if (!errorsgiven)
    errors = new Matrices(layernum + 1);
  else
    errors->resize(layernum + 1);
  if (!zetasgiven)
    zetas = new Matrices(layernum);
  else
    zetas->resize(layernum);
...
const Matrix
ControllerNet::forwardprojection(const matrix::Matrix& error,
                                 Matrices* errors,
                                 Matrices* zetas) const {
  assert(initialised);
  unsigned int layernum = layers.size();
...
const Matrix
ControllerNet::backpropagationX(const Matrix& error,
                                Matrices* errors,
                                Matrices* zetas,
...
const Matrix&
ControllerNet::response() const {
  assert(initialised);
  return L;
}

const Matrix&
ControllerNet::responseLinear() const {
  assert(initialised);
  return R;
}

void
ControllerNet::calcResponseIntern() {
  assert(initialised);
  unsigned int layernum = layers.size();
...
void
ControllerNet::damp(double damping) {
...
bool
ControllerNet::store(FILE* f) const {
...
bool
ControllerNet::write(FILE* f) const {
  int layernum = layers.size();
  fprintf(f, "%i\n", layernum);
  for (const auto& layer : layers) {
    layer.store(f);
