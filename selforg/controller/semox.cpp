...
void
SeMoX::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
SeMoX::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
SeMoX::fillBuffersAndControl(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
  assert(static_cast<unsigned>(number_sensors) == (this->number_sensors + conf.numContext) &&
...
SeMoX::calcXsi(int delay) {
  ...
  pain =
    (xsi_norm > 5 * xsi_norm_avg) ? 1.0 : 0.0; // pain if large raise in error (arbitrary choice)
}
...
SeMoX::learnModel(int delay) {
  ...
  if (!pain) {
    ...
    if (modelNoise > 0) {
      B_update =
        noiseMatrix(B.getM(), B.getN(), *BNoiseGen, -modelNoise, modelNoise); // noise for bias
    }
...
Matrix
SeMoX::calculateControllerValues(const Matrix& x_smooth) {
...
list<Matrix>
SeMoX::getParameters() const {
  return { C, H };
}

int
SeMoX::setParameters(const list<Matrix>& params) {
  if (params.size() == 2) {
...
SeMoX::getStructuralLayers() const {
...
SeMoX::getStructuralConnections() const {
...
void
SeMoX::setMotorTeaching(const matrix::Matrix& teaching) {
...
void
SeMoX::setSensorTeaching(const matrix::Matrix& teaching) {
...
matrix::Matrix
SeMoX::getLastMotorValues() {
  return y_buffer[(t - 1 + buffersize) % buffersize];
}

matrix::Matrix
SeMoX::getLastSensorValues() {
