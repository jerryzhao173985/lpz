...
void
DerPseudoSensor::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  if (conf.useS)
    S.set(number_sensors, number_sensors * 2); // S gets frist and second derivative

  if (conf.sat)
    conf.sat->init(number_sensors, number_motors, conf.cInit);
...
void
DerPseudoSensor::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
DerPseudoSensor::fillBuffersAndControl(const sensor* x_,
                                       int number_sensors,
                                       motor* y_,
                                       int number_motors) {
  ...
  x_smooth +=
    (x - x_smooth) *
    (1.0 / s4avg); // calculateSmoothValues(x_buffer, t < s4avg ? 1 : int(max(1.0,s4avg)));
  ...
  // calculate controller values based on smoothed input
  // values//*.001;//*(tanh(exp(40*grang2)/5)*5);//TEST Granger
...
*/
void
DerPseudoSensor::learnController(int delay) {
  ...
  const Matrix deltaQ =
    (x_buffer[(t) % buffersize] - x_buffer[(t - delay) % buffersize] -
     (RG * x_buffer[(t - delay - 3) % buffersize]) - (Q * x_buffer[(t - delay) % buffersize]) -
     (Q1 * x_buffer[(t - 5 - delay) % buffersize]))
      .map(tanh);
  ...
  if (steps == 2) {
    ...
    CCT_inv =
      ((CST * A * CST) * ((CST * A * CST) ^ T) + (CST * (CST ^ T) * .4).pluslambdaI(0.1)) ^ (-1);
    const Matrix mue = CCT_inv * eta;
    const Matrix chi = mue.multrowwise(g_prime);
    const Matrix rho = mue.multrowwise((z.map(g)).multrowwise(CST * (CST ^ T) * mue)) * -2;
    C_update = ((chi * (chi ^ T) * C) + rho * (x ^ T)) * epsC *
               causalfactor; // * .001;//*(tanh(exp(40*grang2)/5)*5);//TEST Granger
    ...
    C_update +=
      (chi * ((A * kappa1) ^ T) +
       ((kappa2.multrowwise(mue * (-2))).multrowwise(z.map(g)) * (x ^ T)) +
       ((A ^ T) * (C ^ T) * chi).multrowwise(g_prime) * (kappa ^ T) +
       ((((A ^ T) * (C ^ T) * chi).multrowwise(z.map(g))).multrowwise(kappa1 * (-2))) * (x ^ T)) *
      epsC * causalfactor;

    //     C_update +=  (chi * (( A * kappa1 )^T) +
    //     ((kappa2.multrowwise(mue*(-2))).multrowwise(z.map(g))*((A*y)^T))
    //     + ( (A^T)*(C^T) * chi ).multrowwise(g_prime) * (kappa^T) + ((( (A^T)*(C^T) * chi
    //     ).multrowwise(z.map(g))).multrowwise(kappa1*(-2)))*((A*y)^T))*epsC;
    H_update += ((kappa2.multrowwise(mue * (-2))).multrowwise(z.map(g)) +
                 ((((A ^ T) * (C ^ T) * chi).multrowwise(z.map(g))).multrowwise(kappa1 * (-2)))) *
                epsC * causalfactor;
  }
  ...
  conf.sat->learn(x_buffer[(t - delay - 10) % buffersize],
                  y_buffer[(t - 10) % buffersize] * .99,
                  epsSat * causalfactor); // learning with the current causalfactor
  ...
  if (useTeaching) {
    const Matrix& y_local = y_buffer[(t) % buffersize]; // eventuell t-1
    const Matrix& kappa = y_teaching - y_local;
    const Matrix g_p = z.map(g_derivative);
    const Matrix& delta_local = (kappa).multrowwise(g_p);
    C_updateTeaching += (delta_local * (x ^ T)) * teacher; // * epsC;
    H_updateTeaching += delta_local * teacher;             // * epsC;
    // C_updateTeaching += ( (y_buffer[(t)% buffersize])*(x^T) ) * teacher;// * epsC;
    // H_updateTeaching +=  y_buffer[(t)% buffersize]* teacher;// * epsC;

    useTeaching =
      false; // false; after we applied teaching signal it is switched off until new signal is given
  }
  //}
...
void
DerPseudoSensor::learnModel(int delay) {
...
DerPseudoSensor::calculateControllerValues(const Matrix& x_smooth) {
  ...
  //     return ySat; }
  //   else {
  return ((C * x_smooth + H).map(g)) * weighting +
         (conf.sat->process(x_smooth) * (1.0 - weighting));
  // }
...
void
DerPseudoSensor::getLastMotors(motor* motors, int len) {
...
bool
DerPseudoSensor::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  if (conf.useS)
...
bool
DerPseudoSensor::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  H.restore(f);
  if (conf.useS)
...
void
DerPseudoSensor::notifyOnChange(const paramkey& key) {
  if (key == "epsA") {
...
DerPseudoSensor::getInternalParamNames() const {
...
DerPseudoSensor::getInternalParams() const {
  list<iparamval> l;
  if (conf.someInternalParams) {
...
DerPseudoSensor::getStructuralLayers() const {
...
DerPseudoSensor::getStructuralConnections() const {
...
void
DerPseudoSensor::setMotorTeachingSignal(const motor* teaching, int len) {
...
void
DerPseudoSensor::setSensorTeachingSignal(const sensor* teaching, int len) {
...
double
DerPseudoSensor::calcMatrixNorm(const Matrix& m) {
