...
InvertMotorSpace::InvertMotorSpace(int buffersize,
                                   double cInit /* = 0.1 */,
...
void
InvertMotorSpace::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
InvertMotorSpace::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
InvertMotorSpace::fillBuffersAndControl(const sensor* x_,
                                        int number_sensors,
                                        motor* y_,
                                        int number_motors) {
...
void
InvertMotorSpace::learnController(const Matrix& x, const Matrix& x_smooth, int delay) {
...
// normal delta rule
void
InvertMotorSpace::learnModel(const Matrix& x, const Matrix& y) {
  Matrix xsi = x - (A * y + B);
...
Matrix
InvertMotorSpace::calculateControllerValues(const Matrix& x_smooth) {
  return (C * x_smooth + H).map(g);
};

/** stores the controller values to a given file. */
bool
InvertMotorSpace::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  A.store(f);
...
bool
InvertMotorSpace::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  H.restore(f);
  A.restore(f);
...
InvertMotorSpace::getStructuralLayers() const {
...
InvertMotorSpace::getStructuralConnections() const {
