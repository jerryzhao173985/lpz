...
void
InvertMotorBigModel::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  if (conf.useS)
    S.set(number_sensors, number_sensors * 2); // S gets first and second derivative
...
void
InvertMotorBigModel::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
InvertMotorBigModel::stepNoLearning(const sensor* x,
                                    int number_sensors,
                                    motor* y,
                                    int number_motors) {
...
void
InvertMotorBigModel::fillBuffersAndControl(const sensor* x_,
                                           int number_sensors,
                                           motor* y_,
                                           int number_motors) {
...
void
InvertMotorBigModel::calcEtaAndBufferIt(int delay) {
  ...
  if (relativeE) { // divide eta by |y|  == relative error
...
void
InvertMotorBigModel::updateCandH(const Matrix& C_update,
...
void
InvertMotorBigModel::learnModel(int delay) {
...
Matrix
InvertMotorBigModel::calculateControllerValues(const Matrix& x_smooth) {
  return (C * x_smooth + H).map(g);
};

void
InvertMotorBigModel::getLastMotors(motor* motors, int len) {
  const Matrix& y = y_buffer[t % buffersize];
...
bool
InvertMotorBigModel::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  conf.model->store(f);
...
bool
InvertMotorBigModel::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  H.restore(f);
...
InvertMotorBigModel::getInternalParamNames() const {
...
InvertMotorBigModel::getInternalParams() const {
  list<iparamval> l;
  if (conf.someInternalParams) {
...
InvertMotorBigModel::getStructuralLayers() const {
...
InvertMotorBigModel::getStructuralConnections() const {
...
void
InvertMotorBigModel::setMotorTeachingSignal(const motor* teaching, int len) {
...
void
InvertMotorBigModel::setSensorTeachingSignal(const sensor* teaching, int len) {
