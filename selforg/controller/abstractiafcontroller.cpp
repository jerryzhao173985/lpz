...
AbstractIAFController::AbstractIAFController(const AbstractIAFControllerConf& conf)
  : AbstractController("AbstractIAFController", "$Id$")
  , conf(conf)
  , randG(nullptr)
  , initialised(false)
  , sensorNumber(0)
...
void
AbstractIAFController::init(int sensornumber, int motornumber, RandGen* randGen) {
  if (!randGen)
...
void
AbstractIAFController::step(const sensor* sensors,
                            int sensornumber,
                            motor* motors,
                            int motornumber) {
...
void
AbstractIAFController::stepNoLearning(const sensor* sensors,
                                      int sensornumber,
                                      motor* motors,
                                      int motornumber) {
  forwardStep(sensors, sensornumber, motors, motornumber);
...
void
AbstractIAFController::notifyOnChange(const paramkey& key) {
  if (key == "thresholdi") {
...
void
AbstractIAFController::forwardStep(const sensor* sensors,
                                   int number_sensors,
                                   motor* motors,
                                   int number_motors) {
  ...
  for (int i = 0; i < *conf.numberIAFNeuronsPerOutput; i++) {
    for (int j = 0; j < motorNumber; j++) {
      ...
      m.val(0, j) -= y.val(
        0,
        (matrix::I)(motorNumber * i + j + motorNumber * (*conf.numberIAFNeuronsPerOutput))); // neg
    }
  }
...
void
AbstractIAFController::initMatrices() {
