...
ESN::init(unsigned int inputDim, unsigned int outputDim, double unit_map, RandGen* randGen) {
  ...
  //                                 random_minusone_to_one(0)*conf.connectionStrength;
  //                         }
  //         }
...
const Matrix
ESN::process(const Matrix& input) {
  assert(initialized);
...
const Matrix
ESN::learn(const Matrix& input, const Matrix& nom_output, double learnRateFactor) {
  ...
  // // if we want to force a sparse output connections we could restrict them here: (initialization
  // matters!) const Matrix& update = (delta * (ESNState^T)) * (learnRateFactor *
  // conf.learningrate); outputWeights.toMap2(apply,update);
  return output;
}
...
const Matrix
ESN::inversion(const matrix::Matrix& input, const matrix::Matrix& xsi) const {
  // todo
  return Matrix();
}

void
ESN::damp(double damping) // Damp is Dumb
{}

unsigned int
ESN::getInputDim() const {
  return inputWeights.getM();
}

unsigned int
ESN::getOutputDim() const {
  return outputWeights.getN();
}

bool
ESN::store(FILE* f) const {
  // save matrix values
