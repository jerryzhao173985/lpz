...
MultiReinforce::MultiReinforce(const MultiReinforceConf& _conf)
  : AbstractController("MultiReinforce", "$Id: ")
  , number_sensors(0)
  , number_motors(0)
  ...
  //   addParameter(__PLACEHOLDER_3__,&conf.tauE1);
  //   addParameter(__PLACEHOLDER_4__,&conf.tauH);
  //   addParameter(__PLACEHOLDER_5__,&conf.tauI);
...
MultiReinforce::~MultiReinforce() {
  ...
  if (conf.qlearning)
    delete conf.qlearning;
}

void
MultiReinforce::init(int sensornumber, int motornumber, RandGen* randGen) {
...
MultiReinforce::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
  ...
  if (t > buffersize) {
    ...
    if (ts < slidingtime && action != oldaction) {
      ...
      (y_sat2 * (1 - (phasecnt / slidingtime)) + y_sat * (phasecnt / slidingtime))
        .convertToBuffer(y_, number_motors);
    } else {
  ...
  fillMotorBuffer(y_, number_motors); // store the plain c-array __PLACEHOLDER_13__ into the y buffer
...
//     //    cout << phase << __PLACEHOLDER_14__ << action << endl;
//     switch(phase){
...
//         const Matrix& y_sat2 = out2.rows(x_t.getM(), out2.getM()-1);
//         (y_sat1*(1-(phasecnt/slidingtime)) + y_sat2*(phasecnt/slidingtime)).convertToBuffer(y_,
//         number_motors); // store the values into y_ array
//       }else{
//         const Matrix& out = sats[action].net->process(satInput);
//         const Matrix& y_sat = out.rows(x_t.getM(), out.getM()-1);
//         y_sat.convertToBuffer(y_, number_motors); // store the values into y_ array
...
void
MultiReinforce::stepNoLearning(const sensor* x, int number_sensors, motor* y, int number_motors) {
...
void
MultiReinforce::fillSensorBuffer(const sensor* x_, int number_sensors) {
...
void
MultiReinforce::fillMotorBuffer(const motor* y_, int number_motors) {
...
void
MultiReinforce::setManualControl(bool mControl, int action_) {
  if (mControl) {
...
void
MultiReinforce::notifyOnChange(const paramkey& key) {
  if (key == "mancontrol") {
...
bool
MultiReinforce::store(FILE* f) const {
  fprintf(f, "%i\n", conf.numSats);
  fprintf(f, "%i\n", conf.numContext);
...
bool
MultiReinforce::restore(FILE* f) {
  if (!initialised)
    init(2, 2);

  char buffer[128];
  if (fscanf(f, "%127s\n", buffer) != 1)
    return false; // Security fix: added field width limit
...
list<string>
MultiReinforce::createFileList(const char* filestem, int n) {
  list<string> fs;
  for (int i = 0; i < n; ++i) {
    char fname[256];
...
MultiReinforce::getInternalParamNames() const {
...
MultiReinforce::getInternalParams() const {
  list<iparamval> l;
...
MultiReinforce::getStructuralLayers() const {
  list<Inspectable::ILayer> l;
  //   l+=ILayer(__PLACEHOLDER_36__,__PLACEHOLDER_37__, number_sensors, 0, __PLACEHOLDER_38__);
  //   l+=ILayer(__PLACEHOLDER_39__,__PLACEHOLDER_40__, number_motors, 1, __PLACEHOLDER_41__);
  //   l+=ILayer(__PLACEHOLDER_42__,__PLACEHOLDER_43__, number_sensors, 2, __PLACEHOLDER_44__);
  return l;
}

list<Inspectable::IConnection>
MultiReinforce::getStructuralConnections() const {
  list<Inspectable::IConnection> l;
  //   l+=IConnection(__PLACEHOLDER_45__, __PLACEHOLDER_46__, __PLACEHOLDER_47__);
  //   l+=IConnection(__PLACEHOLDER_48__, __PLACEHOLDER_49__, __PLACEHOLDER_50__);
  //   if(conf.useS) l+=IConnection(__PLACEHOLDER_51__, __PLACEHOLDER_52__, __PLACEHOLDER_53__); // this is not quite true! it is x' x'' -> xp
  return l;
}
