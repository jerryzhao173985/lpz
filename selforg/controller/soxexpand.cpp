...
SoxExpand::SoxExpand(const SoxExpandConf& conf)
  ...
  addParameterDef("harmony", &harmony, 0, "dynamical harmony between internal and external world");
...
void
SoxExpand::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
SoxExpand::setA(const matrix::Matrix& _A) {
...
void
SoxExpand::setC(const matrix::Matrix& _C) {
  assert(C.getM() == _C.getM() && C.getN() == _C.getN());
  C = _C;
}

matrix::Matrix
SoxExpand::getContextC() {
  return conf.contextCoupling;
}
void
SoxExpand::setContextC(const matrix::Matrix& CC) {
...
void
SoxExpand::seth(const matrix::Matrix& _h) {
...
void
SoxExpand::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
