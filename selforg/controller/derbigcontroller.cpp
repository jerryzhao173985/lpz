...
DerBigController::DerBigController(const DerBigControllerConf& conf)
  : InvertMotorController(conf.buffersize, "DerBigController", "$Id$")
  , conf(conf) {
...
void
DerBigController::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
DerBigController::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
DerBigController::fillBuffersAndControl(const sensor* x_,
                                        int number_sensors,
                                        motor* y_,
                                        int number_motors) {
  ...
  x_smooth = calculateSmoothValues(x_buffer.get(), t < s4avg ? 1 : int(max(1.0, s4avg)));
...
*/
void
DerBigController::learnController(int delay) {
  ...
  for (unsigned int s = 1; s <= steps; ++s) {
    ...
    // const Matrix& y_local    = y_buffer[(t-s-delay)%buffersize];
    const Matrix& z = (C * x + H); // * weighting +( R * y_local + H) * (1 - weighting);
    ...
    // //     C_update += (( (  chi * (v^T ) ) /*- (zeta * (chi^T))*/  )*(A^T) -  rho*(x^T) ) *
    // epsC;
    // //     // C_update += (( (  chi * (v^T ) ) /*- (zeta * (chi^T))*/  )*(A^T) - rho*(x^T) ) *
    // Dinverse * epsC ;
    // //     // C_update += /*z*/eta * (x^T) * teacher;//Wird jetzt extra gelernt, siehe
    // useTeaching
    ...
    if (useTeaching && s == 1) {
      const Matrix& y_current = y_buffer[(t) % buffersize]; // eventuell t-1
      const Matrix& kappa = y_teaching - y_current;
      const Matrix g_p = z.map(g_s);
      ...
      useTeaching = false; // false; after we applied teaching signal it is switched off until new
                           // signal is given
    }
  }
...
void
DerBigController::learnModel(int delay) {
  ...
  if (conf.useS) {
    const Matrix& x_primes = calcDerivatives(x_buffer.get(), 1);
...
Matrix
DerBigController::calculateControllerValues(const Matrix& x_smooth) {
  return (C * x_smooth + H).map(g);
  // return (C* (x_smooth - x_smooth_long) +H).map(g); //TEST
};

void
DerBigController::getLastMotors(motor* motors, int len) {
  const Matrix& y = y_buffer[t % buffersize];
  y.convertToBuffer(motors, len);
}

Matrix
DerBigController::calcDerivatives(const matrix::Matrix* buffer, int delay) {
...
bool
DerBigController::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  if (conf.useS)
...
bool
DerBigController::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  H.restore(f);
  if (conf.useS)
...
void
DerBigController::notifyOnChange(const paramkey& key) {
  if (key == "epsA") {
...
DerBigController::getInternalParamNames() const {
...
DerBigController::getInternalParams() const {
  list<iparamval> l;
  if (conf.someInternalParams) {
...
DerBigController::getStructuralLayers() const {
...
DerBigController::getStructuralConnections() const {
...
void
DerBigController::setMotorTeachingSignal(const motor* teaching, int len) {
...
void
DerBigController::setSensorTeachingSignal(const sensor* teaching, int len) {
...
double
DerBigController::calcMatrixNorm(const Matrix& m) {
