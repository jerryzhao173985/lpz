...
void
InvertMotorNStep::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  if (conf.useS)
    S.set(number_sensors, conf.numberContext);
  // S gets first and second derivative of context sensors
  if (conf.useSD)
    SD.set(number_sensors, number_sensors * 2);
  ...
  if (conf.initialC.getM() != number_motors || conf.initialC.getN() != number_sensors) {
    if (!conf.initialC.isNulltimesNull())
      cerr << "dimension of Init C are not correct, default is used! \n";
...
void
InvertMotorNStep::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
InvertMotorNStep::fillBuffersAndControl(const sensor* x_,
                                        int number_sensors,
                                        motor* y_,
                                        int number_motors) {
  ...
  if (activeExplore != 0) { // through model backpropagated error used as control signal for robot
...
double
InvertMotorNStep::regularizedInverse(double v) {
  return 1 / (fabs(v) + 0.1);
}
...
void
InvertMotorNStep::calcEtaAndBufferIt(int delay) {
...
void
InvertMotorNStep::calcXsi(int delay) {
...
void
InvertMotorNStep::learnController(int delay) {
...
InvertMotorNStep::calcCandHUpdates(Matrix& C_update, Matrix& H_update, int delay) {
  ...
  for (unsigned int s = 1; s <= steps; s++) {
    ...
    //     z                    = R * z.map(g) + H + C*xsi; // z is a dynamic itself (not
    //     successful)
    const Matrix& x = x_buffer[(t - s) % buffersize];
    const Matrix& z = C * x + H;
    const Matrix shift =
      (eta + v).mapP(&shiftlimit, squash); // limit shift by shiftlimit (arbitrary choice)
    ...
    if (continuity != 0 && s == 1) { // learning to keep motorcommands smooth
      ...
      const Matrix& xsi_tle = y_tm2 - y_tm1;
      const Matrix& delta = xsi_tle.multrowwise(g_prime);
    ...
    if (useTeaching && s == 1) {
      ...
      C_updateTeaching += (LLT_I * delta_teach * (x ^ T)) * (teacher * epsC);
      H_updateTeaching += (LLT_I * delta_teach) * (teacher * epsC);
      useTeaching =
        false; // after we applied teaching signal it is switched off until new signal is given
    }
  }
...
InvertMotorNStep::calcCandHUpdatesTeaching(Matrix& C_update, Matrix& H_update, int y_delay) {
  ...
  const Matrix v_local = ((R + SmallID) ^ -1) * zeta;
...
void
InvertMotorNStep::updateCandH(const Matrix& C_update, const Matrix& H_update, double _squashSize) {
...
void
InvertMotorNStep::learnModel(int delay) {
  const Matrix& y = y_buffer[(t - 1 - delay) % buffersize];
  ...
  if (xsi_norm > 5 * xsi_norm_avg) {
    ...
    if (conf.useS) {
      ...
      const Matrix& x_c = x_buffer[(t - 1) % buffersize].rows(number_sensors - conf.numberContext,
                                                              number_sensors - 1);
    ...
    if (noiseB > 0) {
      Matrix B_noise =
        noiseMatrix(B.getM(), B.getN(), *BNoiseGen, -noiseB, noiseB); // noise for bias
      B_update += B_noise;
    }
    ...
    if (adaptRate != 0) {
      ...
      epsA = adaptMinMax(
        epsA, matrixNorm1(A_update), normA / 2500, normA / 250, adaptRate, adaptRate * 5);
      epsA = min(2.0, epsA);
    }
...
Matrix
InvertMotorNStep::calculateControllerValues(const Matrix& x_smooth) {
  return (C * x_smooth + H).map(g);
};

void
InvertMotorNStep::getLastMotors(motor* motors, int len) {
  const Matrix& y = y_buffer[(t - 1) % buffersize];
  y.convertToBuffer(motors, len);
}

void
InvertMotorNStep::getLastSensors(sensor* sensors, int len) {
  const Matrix& x = x_buffer[t % buffersize];
  x.convertToBuffer(sensors, len);
}

void
InvertMotorNStep::setSensorWeights(const Matrix& weights) {
  assert(initialised);
...
bool
InvertMotorNStep::store(FILE* f) const {
  // save matrix values
  C.store(f);
  H.store(f);
  A.store(f);
...
bool
InvertMotorNStep::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  H.restore(f);
  A.restore(f);
...
InvertMotorNStep::getStructuralLayers() const {
...
InvertMotorNStep::getStructuralConnections() const {
  list<Inspectable::IConnection> l;
  l += IConnection("C", "x", "y");
  l += IConnection("A", "y", "xP");
  if (conf.useS)
    l += IConnection("S", "x", "xP");
  return l;
}

double
InvertMotorNStep::clip095(double x) {
  return clip(x, -0.95, 0.95);
}

void
InvertMotorNStep::setMotorTeachingSignal(const motor* teaching, int len) {
...
void
InvertMotorNStep::setSensorTeachingSignal(const sensor* teaching, int len) {
...
void
InvertMotorNStep::setMotorTeaching(const matrix::Matrix& teaching) {
...
void
InvertMotorNStep::setSensorTeaching(const matrix::Matrix& teaching) {
...
matrix::Matrix
InvertMotorNStep::getLastMotorValues() {
  return y_buffer[(t - 1 + buffersize) % buffersize];
}

matrix::Matrix
InvertMotorNStep::getLastSensorValues() {
  return x_buffer[(t - 1 + buffersize) % buffersize];
}

void
InvertMotorNStep::setReinforcement(double reinforcement) {
