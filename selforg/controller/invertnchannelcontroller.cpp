 ...
 *   this license, visit http:__PLACEHOLDER_25__
...
void
InvertNChannelController::init(int sensornumber, int motornumber, RandGen* randGen) {
...
void
InvertNChannelController::step(const sensor* x_, int number_sensors, motor* y_, int number_motors) {
...
void
InvertNChannelController::stepNoLearning(const sensor* x_,
                                         int number_sensors,
                                         motor* y_,
                                         int number_motors) {
  assert(static_cast<unsigned>(number_sensors) <= number_channels && static_cast<unsigned>(number_motors) <= number_channels);
...
double
InvertNChannelController::calculateE(const Matrix& x_delay, const Matrix& y_delay) {
  ...
  //       for (int j = 0; j < number_channels; ++j)
  //         {
  //           E_s += (A[i][j]*g(z[j]) - x_buffer[(t+buffersize)%buffersize][i]) * (A[i][j]*g(z[j])
  //           - x_buffer[(t+buffersize)%buffersize][i]);
  //         }
  //     }
...
/// learn values h,C,A
void
InvertNChannelController::learn(const Matrix& x_delay, const Matrix& y_delay) {
...
// normal delta rule
void
InvertNChannelController::learnmodel(const Matrix& y_delay) {
  Matrix xsi = x_buffer[t % buffersize] - A * y_delay;
...
Matrix
InvertNChannelController::calculateControllerValues(const Matrix& x_smooth) {
...
bool
InvertNChannelController::store(FILE* f) const {
  // save matrix values
  C.store(f);
  h.store(f);
  A.store(f);
...
bool
InvertNChannelController::restore(FILE* f) {
  // save matrix values
  C.restore(f);
  h.restore(f);
  A.restore(f);
...
InvertNChannelController::getStructuralLayers() const {
...
InvertNChannelController::getStructuralConnections() const {
