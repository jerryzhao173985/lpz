...
MultiLayerFFNN::MultiLayerFFNN(double eps,
...
void
MultiLayerFFNN::init(unsigned int inputDim,
  ...
  for (unsigned int i = 1; i < layers.size(); ++i) {
    ...
    w_tmp = w_tmp.mapP(randGen, random_minusone_to_one) * 0.05 +
            (useBypass ? w_tmp : (w_tmp ^ 0) * unit_map);
    weights[i] = w_tmp;
  ...
  if (useBypass) {
    ...
    bypassWeights =
      bypassWeights.mapP(randGen, random_minusone_to_one) * 0.05 + (bypassWeights ^ 0) * unit_map;
  }
...
const Matrix
MultiLayerFFNN::process(const Matrix& input) {
  assert(initialised);
  unsigned int layernum = layers.size();
...
const Matrix
MultiLayerFFNN::learn(const Matrix& input, const Matrix& nom_output, double learnRateFactor) {
  assert(initialised);
...
const Matrix
MultiLayerFFNN::response(const Matrix& input) const {
  assert(initialised);
  unsigned int layernum = layers.size();
...
bool
MultiLayerFFNN::store(FILE* f) const {
  fprintf(f, "%g\n", eps);
  int layernum = layers.size();
  fprintf(f, "%i\n", layernum);
...
bool
MultiLayerFFNN::write(FILE* f) const {
  fprintf(f, "%g\n", eps);
  int layernum = layers.size();
  fprintf(f, "%i\n", layernum);
  for (int i = 0; i < layernum; ++i) {
...
MultiLayerFFNN::restore(FILE* f) {
  ...
  if (fread(&useBypass, sizeof(bool), 1, f) != 1)
    return false;
  if (useBypass)
...
MultiLayerFFNN::getInternalParamNames() const {
...
MultiLayerFFNN::getInternalParams() const {
  list<iparamval> l;
  int weightsnum = weights.size();
  for (int i = 0; i < weightsnum; ++i) {
    if (someInternalParams)
...
MultiLayerFFNN::getStructuralLayers() const {
...
MultiLayerFFNN::getStructuralConnections() const {
...
MultiLayerFFNN::setActivationFunction(ActivationFunction actfun) {
