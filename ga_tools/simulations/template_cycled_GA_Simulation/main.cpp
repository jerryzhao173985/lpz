...
public:
      ...
      GlobalData& global)
  {
    ...
    gSStr = SingletonGenAlgAPI::getInstance()->createFixGenerationSizeStrategy(static_cast<int>((numberIndividuals - (numberIndividuals / 10)) / 2));
  ...
  void explicit createBots(const GlobalData& global)
  {
    ...
    for (int ind = 0; ind < numberIndividuals; ++ind)
    {
      ...
      vehicle->place(Pos(static_cast<double>(ind % static_cast<int>(sqrt(numberIndividuals))) * 19.0,
          19.0 * static_cast<double>(ind / static_cast<int>(sqrt(numberIndividuals))), 0.0));
