...
class InspectableProxy;
class PlotOptionEngine;

/**
 * Singleton engine for genetic algorithms
 */
...
public:
	 ...
	 */
	inline int getNumGeneration(void) const {return static_cast<int>(m_generation.size());}
	 ...
	 */
	inline Generation* getGeneration(int x) {if(x<getNumGeneration())return m_generation[x];return nullptr;}
	...
	inline Generation* getActualGeneration(void) const {return m_generation[m_actualGeneration];}

	/**
	 * returns the number of individual inside the alg.
	 * @return (int) number of individual
	 */
	inline int getNumIndividual(void) const {return static_cast<int>(m_individual.size());}
	 ...
	 */
	inline Individual* getIndividual(int x) const {if(x<getNumIndividual())return m_individual[x];return nullptr;}
	 ...
	 */
	inline Gen* getGen(int x)const {if(static_cast<unsigned int>(x)<m_gen.size())return m_gen[x];return nullptr;}
	 ...
	 */
	void prepare(int startSize, int numChildren, InspectableProxy*& proxyGeneration, InspectableProxy*& proxyGene, RandGen* random, PlotOptionEngine* plotEngine = nullptr, PlotOptionEngine* plotEngineGenContext = nullptr, bool withUpdate = true);

	/**
	 * return the size of the next generation.
	 * @return (int) the size
	 */
	int getNextGenerationSize() const;
	 ...
	 * @param proxyGene (InspectableProxy*&) the proxy for control measure
	 */
	void measureStep(double time,InspectableProxy*& proxyGeneration, InspectableProxy*& proxyGene, PlotOptionEngine* plotEngine = nullptr, PlotOptionEngine* plotEngineGenContext = nullptr);
	 ...
	 * @param plotEngineGenContext (PlotOptionEngine*) logging the data of the GenContext for later control
	 */
	void runGenAlg(int startSize, int numChildren, int numGeneration, RandGen* random, PlotOptionEngine* plotEngine = nullptr, PlotOptionEngine* plotEngineGenContext = nullptr);
	 ...
	 */
	inline static void destroyGenEngine(bool cleanStrategies=false) {getInstance()->m_cleanStrategies=cleanStrategies; if(m_engine!= nullptr){delete m_engine;m_engine=nullptr;}}
